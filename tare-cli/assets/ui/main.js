...
import { icon, calibrationMark } from "./ui/icon.js";
import { errorNode } from "./ui/errorNode.js";
import { skelScreen } from "./ui/skeleton.js";
import { onRoute, routeHash, routePath, hashOf, navigate } from "./ui/store.js";
...
import { investigationFromState, migratePrefsToState, routeToWorkspace, } from "./analysis/investigation.js";
import { SECTIONS, LENS_ROUTES, FOOTER, allNav, sectionFor, } from "./shell/rail.js";
...
import { initialTheme, applyTheme, watchOsTheme, initialIdentity, applyIdentity, } from "./ui/theme.js";
import { isOnboarded, density, applyDensity, rangePref, setRangePref, recentRuns, pinnedRuns, baselineRun, reconcileRunReferences, defaultScreen, } from "./ui/prefs.js";
import { RANGE_OPTIONS, rangeToWindow } from "./ui/range.js";
import { openPalette, installPaletteHotkey } from "./ui/palette.js";
import { formatShortcut, matchShortcut } from "./ui/keymap.js";
import { osClass } from "./ui/os.js";
import { ACTION, navId, runId, orderCommands, selectionCommands, } from "./commands/registry.js";
...
function routeHonorsRange(route) {
...
function baseLabel(name) {
    const known = allNav().find((n) => n.name === name)?.label ??
...
function canonicalViewTitle(route) {
    if (route.name === "investigate" && route.segments[1] === "run" && route.segments[2]) {
        return `Run · ${route.segments[2]}`;
    }
    if (route.name === "investigate" && route.segments[1] === "compare")
        return "Compare runs";
    const query = route.query ?? {};
    if (route.name === "investigate") {
        if (query.view === "units")
            return "Investigate · Work units";
        if (query.mode === "lineage")
            return "Investigate · Prompt lineage";
        if (query.mode === "distinguish")
            return "Investigate · Correlations";
        if (query.mode === "timeline")
            return "Investigate · Timeline";
    ...
    if (route.name === "optimize" && query.view === "scenarios")
        return "Optimize · Scenarios";
    if (route.name === "pulse" && query.mode === "now")
        return "Pulse · Now";
    return labelFor(route);
}
function investigationScopeLabel(inv) {
...
function investigationHref(inv) {
...
function setBreadcrumb(nav, route) {
    const nestedRunId = route.name === "investigate" && route.segments[1] === "run" ? route.segments[2] : undefined;
    if (nestedRunId) {
        const { view: _view, ...scopeQuery } = route.query ?? {};
        const segs = [];
        const sec = sectionFor("investigate");
        if (sec && sec !== baseLabel("investigate"))
            segs.push({ label: sec });
        segs.push({
...
function placeholder(label) {
    return async (root) => {
        ...
        root.replaceChildren(el("section", { class: "section" }, [
...
export async function mountApp(root, client) {
    ...
    if (investigationsResult.status === "fulfilled")
        savedInvestigations = investigationsResult.value;
    if (runsResult.status === "fulfilled")
        reconcileRunReferences(runsResult.value);
    clear(root);
    ...
    const os = document.documentElement.dataset.os || osClass(navigator.userAgent);
    ...
    const navDrawerTrigger = el("button", {
    ...
    const navDrawerClose = el("button", {
        class: "btn ghost nav-drawer-close",
        type: "button",
        "aria-label": "Close navigation",
    }, [icon("close", { size: 16 })]);
    navEl.appendChild(el("header", { class: "nav-drawer-header" }, [
    ...
    for (const sec of SECTIONS) {
        ...
        for (const it of sec.items)
            list.appendChild(el("li", {}, [mkLink(it)]));
    ...
    const renderViews = () => {
        ...
        const children = [el("p", { class: "nav-section", text: "Views" })];
        if (views.length === 0) {
    ...
    const runNavMeta = new Map();
    let runNavHydrationKey = "";
    const hydrateRunsNav = async (ids) => {
        const unique = [...new Set(ids)].slice(0, 12);
        const key = unique.join("\u0000");
        if (!key || key === runNavHydrationKey)
            return;
    ...
    const openSettingsInCurrentWorkspace = () => {
    ...
    const openTrustInCurrentWorkspace = (pricing = false) => {
        if (currentRoute && isWorkspaceRoute(currentRoute.name)) {
            const query = { ...(currentRoute.query ?? {}), sheet: "trust" };
            if (pricing) {
    ...
    const openCaptureInCurrentWorkspace = () => {
        if (currentRoute && isWorkspaceRoute(currentRoute.name)) {
            window.location.hash = routePath(currentRoute.segments, {
                ...(currentRoute.query ?? {}),
                sheet: "capture",
    ...
    const rangeSel = el("select", { class: "range-select", "aria-label": "Analysis time range" }, RANGE_OPTIONS.map((o) => el("option", { value: o.key, text: o.label, ...(o.key === initialRange.key ? { selected: "" } : {}) })));
    const fromInput = el("input", { type: "date", class: "range-date", "aria-label": "From date", value: initialRange.from ?? "" });
    const toInput = el("input", { type: "date", class: "range-date", "aria-label": "To date", value: initialRange.to ?? "" });
    ...
    const latestCapturedDay = async () => {
        if (rangeAnchor)
            return rangeAnchor;
        try {
            const trend = await client.trend({ by: "total" });
            if (validDate(trend.to))
                rangeAnchor = trend.to;
        }
        catch {
            /* An empty/unavailable store falls back to today's UTC date for a well-formed initial scope. */
        }
        rangeAnchor ?? (rangeAnchor = new Date().toISOString().slice(0, 10));
        return rangeAnchor;
    };
    const materializeRange = async (pref) => {
    ...
    const rangeKeys = new Set(["7d", "30d", "90d", "custom"]);
    const syncRangeControl = (query, scope) => {
    ...
    const applyRange = async () => {
        if (!currentRoute || !routeHonorsRange(currentRoute))
            return;
        ...
        if (mine !== applyRangeToken || !currentRoute)
            return;
        setRangePref(pref);
    ...
    rangeSel.addEventListener("change", () => {
        ...
        if (rangeSel.value === "custom") {
            ...
            fromInput.value || (fromInput.value = scope.from ?? "");
            toInput.value || (toInput.value = scope.to ?? "");
    ...
    void (async () => {
        try {
            ...
            if (cfg?.privacy?.profile === "max_inspect")
                captureBanner.removeAttribute("hidden");
        }
        catch {
    ...
    const drawerFocusable = 'a[href], button:not([disabled]), input:not([disabled]), select:not([disabled]), textarea:not([disabled]), details > summary, [tabindex]:not([tabindex="-1"])';
    const drawerBackground = [skipLink, brand, topbar, main, statusbar];
    let navDrawerOpen = false;
    const closeNavDrawer = (restoreFocus = true) => {
        if (!navDrawerOpen)
            return;
        navDrawerOpen = false;
        ...
        afterTransition(navEl, "transform", () => {
            ...
            for (const node of drawerBackground)
    ...
    navEl.addEventListener("keydown", (event) => {
        ...
        if (event.key !== "Tab")
            return;
        const focusable = Array.from(navEl.querySelectorAll(drawerFocusable)).filter((node) => !node.hasAttribute("hidden") &&
    ...
    observeWorkbench(shell, (mode) => {
        ...
        if (mode === "full" || mode === "rail-icons")
            closeNavDrawer(false);
        main.querySelector("[data-adaptive-panes]")?.dispatchEvent(new CustomEvent("tare:layoutchange", { detail: mode }));
    });
    const ALERT_SEEN_KEY = "tare.alerts.seen.v1";
    let alertPollInFlight = false;
    const readSeenAlertKeys = () => {
        try {
    ...
    const dayAtOffset = (offset) => {
        ...
        return new Date(Date.now() + safe * 60000).toISOString().slice(0, 10);
    };
    ...
    const refreshAlerts = async (today) => {
        ...
        try {
            ...
            const [budget, burnrate, anomalies, statuses] = await Promise.all([
                ...
                needsRunRate ? client.burnrate().catch(() => undefined) : undefined,
                client.anomalies({ by: "total" }).catch(() => []),
            ...
            const anomaliesByWindow = {};
            await Promise.all(windows.map(async (window) => {
                anomaliesByWindow[String(window)] = await client
                    .anomalies({ by: "total", window })
                    .catch(() => []);
            }));
            const capturedEvents = statuses.reduce((total, status) => Math.min(Number.MAX_SAFE_INTEGER, total + Math.max(0, status.steps)), 0);
            ...
            if (remaining > 0) {
                ...
                if (client.canNotify())
                    void client.notify("Tare alerts", message).catch(() => undefined);
            }
        }
        catch {
    ...
    // ---- routing ----
    let renderToken = 0;
    async function renderActive(route) {
        ...
        main.querySelector("[data-adaptive-panes]")?.dispatchEvent(new Event("tare:dispose"));
        ...
        if (!rangeControl.hidden)
            syncRangeControl(route.query);
        ...
        try {
            ...
            if (mine === renderToken) {
                main.replaceChildren(errorNode("Couldn't render this screen. Your current view and filters are unchanged.", e, {
                    actions: [
                        {
                            label: "Retry",
                            primary: true,
    ...
    function selectionFromRoute(r) {
        if (!r)
            return null;
        if (r.name === "runs" && r.param)
            return { kind: "run", id: r.param };
    ...
    async function buildCommands() {
        ...
        const selection = selectionCommands({ selection: selectionFromRoute(currentRoute) }, {
            open: (id) => {
        ...
        const nav = SECTIONS.flatMap((section) => section.items).map((n) => ({
        ...
        const savedViews = savedInvestigations.map((view) => ({
            id: `view:${view.id}`,
            title: view.label,
        ...
        const actions = [
            ...
            },
            {
                ...
                run: async () => {
                    ...
                    const cleanQuery = Object.fromEntries(Object.entries(r?.query ?? {}).filter(([key, value]) => key !== "sheet" && key !== "investigation" && value !== ""));
                    ...
                    if (prior)
                        inv.created_at = prior.created_at;
                    try {
        ...
        const runs = recentRuns().slice(0, 10).map((id) => {
            const meta = runNavMeta.get(id);
            return {
                id: runId(id),
                title: id,
        ...
        const advanced = LENS_ROUTES.map((lens) => ({
            id: `lens:${lens.name}`,
            title: lens.label,
    ...
    const gotoHotkeys = Object.entries(GOTO_CHORDS).map(([name, seq]) => ({
        seq,
    ...
    onRoute((r) => {
        ...
        if (routed !== r)
            window.history.replaceState(null, "", hashOf(routed));
        currentRoute = routed;
