...
function dedupSortRefs(refs) {
    const sorted = [...refs].sort((a, b) => byteCompare(a.run_id, b.run_id) || a.step_ordinal - b.step_ordinal);
    const out = [];
    for (const r of sorted) {
        const last = out[out.length - 1];
        if (!last || last.run_id !== r.run_id || last.step_ordinal !== r.step_ordinal)
            out.push(r);
    }
    return out;
}
...
export function canonicalCohortJson(spec) {
...
export function cohortHash(spec) {
...
const ENTITIES = ["run", "step"];
const METRICS = ["spend_micros", "tokens", "cache_hit_rate"];
const NORMS = [
    "absolute",
    "share_of_selection",
    "per_run",
...
export function decodeFilters(s) {
    const parsed = parseJson(base64urlDecode(s), "f (filters)");
    if (!Array.isArray(parsed))
        throw new AnalysisUrlError("f payload is not a filter array");
    return parsed;
}
/// Encode a CohortSpec for `sel=`/`base=` (base64url of its canonical JSON).
export function encodeCohort(spec) {
...
export function decodeCohort(s, param) {
    const parsed = parseJson(base64urlDecode(s), param);
    if (!parsed ||
...
export function encodeAnalysisQuery(s) {
    const q = {};
    if (s.from)
        q.from = s.from;
    if (s.to)
    ...
    if (s.norm)
        q.norm = s.norm;
    if (s.pricing)
        q.pricing = encodePricing(s.pricing);
    if (s.outcome)
        q.outcome = encodeOutcome(s.outcome);
    if (s.sheet)
        q.sheet = s.sheet;
    if (s.view)
        q.view = s.view;
    if (s.filters && s.filters.length > 0)
        q.f = encodeFilters(s.filters);
    if (s.selection === null)
        q.sel = "none";
    else if (s.selection)
        q.sel = encodeCohort(s.selection);
    if (s.baseline)
        q.base = encodeCohort(s.baseline);
    if (s.baselineKind)
        q.base_kind = s.baselineKind;
    if (s.baselineSampleCount != null)
        q.base_n = String(s.baselineSampleCount);
    return q;
}
...
export function decodeAnalysisQuery(q) {
    const s = {};
    if (q.from)
        s.from = q.from;
    if (q.to)
    ...
    if (q.tz)
        s.tz = q.tz;
    if (q.entity)
        s.entity = oneOf(q.entity, ENTITIES, "entity");
    if (q.metric)
        s.metric = oneOf(q.metric, METRICS, "metric");
    if (q.norm)
        s.norm = oneOf(q.norm, NORMS, "norm");
    if (q.pricing)
        s.pricing = parsePricing(q.pricing);
    if (q.outcome)
        s.outcome = parseOutcome(q.outcome);
    if (q.sheet)
        s.sheet = q.sheet;
    if (q.view)
        s.view = q.view;
    if (q.f)
        s.filters = decodeFilters(q.f);
    if (q.sel)
        s.selection = q.sel === "none" ? null : decodeCohort(q.sel, "sel");
    if (q.base)
        s.baseline = decodeCohort(q.base, "base");
    if (q.base_kind)
        s.baselineKind = oneOf(q.base_kind, BASELINE_KINDS, "baseline kind");
    if (q.base_n) {
...
export function overflowsHashBudget(hash) {
