...
import { setRouteQuery } from "../ui/store.js";
import { errorNode } from "../ui/errorNode.js";
import { createUtilitySheet } from "../ui/sheet.js";
import { defaultFocus, hydrateFromQuery, } from "../analysis/store.js";
...
const SETTINGS_LOADER = () => import("../screens/settings.js").then((module) => module.renderSettings);
const CAPTURE_LOADER = () => import("../screens/capture.js").then((module) => module.renderCapture);
const TRUST_LOADER = () => import("../screens/trust.js").then((module) => module.renderTrust);
/**
 ...
 */
function inertUtilityBackground(root) {
...
async function renderSettingsSheet(root, client, route) {
    const releaseBackground = inertUtilityBackground(root);
    root.addEventListener("tare:dispose", releaseBackground, { once: true });
    const returnFocus = document.querySelector('[data-utility-sheet-trigger="settings"]');
...
async function renderTrustSheet(root, client, route, context) {
    const releaseBackground = inertUtilityBackground(root);
    root.addEventListener("tare:dispose", releaseBackground, { once: true });
    const returnFocus = document.querySelector('[data-utility-sheet-trigger="trust"]');
    const sheet = createUtilitySheet("Trust & pricing", () => {
        releaseBackground();
        const restoreView = route.query?.view === "pricing" ? route.query?.workspace_view ?? "" : route.query?.view ?? "";
...
async function renderCaptureSheet(root, client) {
    const releaseBackground = inertUtilityBackground(root);
    root.addEventListener("tare:dispose", releaseBackground, { once: true });
    const returnFocus = document.querySelector('[data-utility-sheet-trigger="capture"]');
...
export function isWorkspaceRoute(name) {
...
export async function hydrateWorkspaceAnalysis(store, name, route, client) {
    const investigationId = route.query?.investigation;
    if (investigationId) {
        if (!client)
            throw new Error("saved-investigation client is unavailable");
        const saved = (await client.listInvestigations()).find((row) => row.id === investigationId);
        if (!saved)
            throw new Error(`saved investigation ${investigationId} was not found`);
...
export async function renderWorkspace(name, root, client, route, context) {
    try {
        ...
        if (!route.query?.investigation)
            throw error;
        root.replaceChildren(errorNode("Couldn't open the saved investigation. It may be unavailable or was removed.", error, {
            actions: [
                {
                    label: "Retry",
                    primary: true,
