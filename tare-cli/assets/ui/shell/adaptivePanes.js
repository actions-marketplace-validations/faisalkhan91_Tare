...
export const LAYOUT_BREAKPOINTS = { full: 1280, railIcons: 900, stacked: 600 };
...
export function layoutMode(inlineSize) {
    if (inlineSize >= LAYOUT_BREAKPOINTS.full)
        return "full";
    if (inlineSize >= LAYOUT_BREAKPOINTS.railIcons)
        return "rail-icons";
    if (inlineSize >= LAYOUT_BREAKPOINTS.stacked)
        return "stacked";
    return "single";
}
...
export function inspectorIsPeek(mode) {
...
export function observeWorkbench(container, onMode, read = (el) => el.clientWidth) {
    let last = null;
    const apply = () => {
...
function modeFromRoot(root) {
    const value = root.closest(".shell[data-layout]")?.dataset.layout;
...
export function installAdaptivePaneInteractions(root, options = {}) {
    const analysis = options.analysis;
    const storage = options.storage ?? defaultStorage();
    const panes = {
        entities: root.querySelector('[data-pane="entities"]'),
        canvas: root.querySelector('[data-pane="canvas"]'),
        inspector: root.querySelector('[data-pane="inspector"]'),
    };
    const separators = {
        entities: root.querySelector('[data-resize-pane="entities"]'),
        inspector: root.querySelector('[data-resize-pane="inspector"]'),
    };
    const nav = root.querySelector("[data-pane-nav]");
    const back = root.querySelector("[data-pane-back]");
    const forward = root.querySelector("[data-pane-forward]");
    const title = root.querySelector("[data-pane-title]");
    const inspectorTriggers = Array.from(root.querySelectorAll("[data-pane-inspector]"));
    const widths = {
    ...
    let peekReturnPane = "canvas";
    let disposed = false;
    const saveWidth = (pane) => {
        try {
    ...
    const applyWidth = (pane, value, persist) => {
    ...
    for (const pane of ["entities", "inspector"]) {
        const spec = PANE_WIDTHS[pane];
    ...
    const levels = () => mode === "single" ? NARROW_LEVELS.single : NARROW_LEVELS.stacked;
    const activePane = () => levels()[Math.min(level, levels().length - 1)];
    const paneLabel = (pane) => panes[pane]?.dataset.paneLabel || PANE_LABEL[pane];
    const visible = (pane, show) => {
    ...
    const sync = () => {
        ...
        if (narrow) {
            ...
            if (title)
                title.textContent = paneLabel(activePane());
            if (back) {
    ...
    const applyMode = (next) => {
        ...
        mode = next;
        if (mode === "stacked")
            level = Math.min(level, NARROW_LEVELS.stacked.length - 1);
        if (peek)
            analysis?.setFocus({ pane: "inspector" });
        else if (mode === "stacked" || mode === "single")
            analysis?.setFocus({ pane: activePane() });
        sync();
    };
        ...
        trigger.addEventListener("click", () => {
            if (mode === "single") {
                ...
                if (activePane() === "inspector")
                    void setLevel(Math.max(0, inspectorLevel - 1));
                else
                    void setLevel(inspectorLevel);
            }
    ...
    for (const pane of ["entities", "inspector"]) {
        ...
        separator.addEventListener("keydown", (event) => {
            let next = null;
            if (event.key === "Home")
                next = PANE_WIDTHS[pane].min;
            if (event.key === "End")
                next = PANE_WIDTHS[pane].max;
            if (event.key === "ArrowLeft")
                next = widths[pane] + (pane === "entities" ? -8 : 8);
    ...
    const keyTarget = root.isConnected ? document : root;
    keyTarget.addEventListener("keydown", onKeyDown);
    const onLayout = (event) => {
        const next = event.detail;
        if (next === "full" || next === "rail-icons" || next === "stacked" || next === "single")
            applyMode(next);
    };
    root.addEventListener("tare:layoutchange", onLayout);
    const onPaneLabel = () => sync();
