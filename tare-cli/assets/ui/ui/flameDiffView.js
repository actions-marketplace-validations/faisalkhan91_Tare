...
export function nodeDelta(node, mode) {
...
export function nodeChanged(node, mode) {
...
export function focusNode(model, focusPath) {
    let node = model.root;
    const resolved = [];
...
export function flameDiffRows(model, opts) {
    ...
    const walk = (node, depth, path) => {
        node.children.forEach((child, index) => {
            if (opts.diffOnly && !nodeChanged(child, opts.mode))
                return;
...
function directionText(delta, mode) {
    if (delta === 0)
        return "no change";
    if (mode === "normalized")
        return delta > 0 ? "larger share" : "smaller share";
...
export function flameDiffTree(model, opts) {
    ...
    const diffOnly = el("label", { class: "flame-diff-diffonly" }, [
        (() => {
            const box = el("input", { type: "checkbox", ...(opts.diffOnly ? { checked: "" } : {}) });
    ...
    const renderRow = (row, globalIndex) => {
