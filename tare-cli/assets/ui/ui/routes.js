...
function canon(segments, implied, legacyQuery, drop = []) {
    const merged = { ...(legacyQuery ?? {}) };
    for (const k of drop)
        delete merged[k];
    Object.assign(merged, implied);
    const r = { name: segments[0], segments };
...
export function redirectRoute(route) {
    ...
    switch (route.name) {
        ...
        case "units":
            ...
            return canon(["investigate"], { view: "units", metric: "spend_micros", norm: "per_outcome" }, q);
        case "compare":
        case "diff":
...
export function migrateDefaultScreen(value) {
    switch (value) {
        case "live":
        case "overview":
        case "pulse":
