...
const SUBCENT = 10000; // micros below $0.01
const COMPACT_AT = 1000000 * MICROS_PER_USD; // abbreviate at >= $1,000,000
...
export function fmtUsd(micros) {
    const m = Math.trunc(micros);
    if (!Number.isFinite(m))
        return "N/A"; // NaN/±Infinity never render as "$NaN"
    if (m === 0)
        return "$0.00";
...
export function fmtSignedUsd(micros) {
    const m = Math.trunc(micros);
...
export function fmtSignedDollars(micros) {
