...
function transitionTimeMs(element, watchedProperty) {
    ...
    for (let index = 0; index < count; index += 1) {
        ...
        if (property !== "all" && property !== watchedProperty)
            continue;
 ...
 */
export function beginMotion(element, attribute, initial = "opening", settled = "open") {
    element.setAttribute(attribute, initial);
    queueMicrotask(() => {
        if (element.getAttribute(attribute) !== initial)
            return;
        const settle = () => {
            if (element.getAttribute(attribute) === initial)
                element.setAttribute(attribute, settled);
        };
...
export function afterTransition(element, watchedProperty, complete) {
    let completed = false;
    let fallback;
    const finish = () => {
        if (completed)
    ...
    const onEnd = (event) => {
        ...
        if (event.target === element && transition.propertyName === watchedProperty)
            finish();
    };
