...
function detectedLabels(snapshot) {
    const labels = new Set();
    for (const agent of snapshot.agents.value ?? [])
        labels.add(sourceLabel(agent.source));
    for (const source of snapshot.coverage.value?.sources ?? [])
        labels.add(sourceLabel(source.source));
    return [...labels];
}
function copyButton(text) {
    const button = el("button", { class: "btn ghost", type: "button", text: "Copy configuration" });
...
function stateSection(snapshot) {
...
function privacySection(snapshot) {
...
function recommendationSection(snapshot, client) {
    const state = captureState(snapshot);
...
function checkRow(status, name, message, fix) {
...
function renderChecks(host, snapshot) {
    const coverage = snapshot.coverage.value;
    ...
    const rows = [];
    rows.push(snapshot.coverage.value
    ...
    rows.push(stepCount > 0 || (otlp?.events ?? 0) > 0
    ...
    rows.push(agentLabels.length > 0 || (otlp?.hooks_seen ?? 0) > 0
    ...
    rows.push(snapshot.config.value
    ...
    rows.push(snapshot.pricing.value
...
function selfCheckSection(client) {
    const result = el("div", { class: "capture-checks", "aria-live": "polite" });
    const button = el("button", { class: "btn primary", type: "button", text: "Run self-check" });
    button.addEventListener("click", async () => {
        button.disabled = true;
...
function advancedSection(client) {
...
export async function renderCapture(root, client) {
    ...
    root.replaceChildren(stateSection(snapshot), privacySection(snapshot), recommendationSection(snapshot, client), selfCheckSection(client), el("p", {
