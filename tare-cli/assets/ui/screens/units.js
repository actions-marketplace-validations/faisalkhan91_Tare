...
export async function renderUnits(root, client) {
    const subtitle = "Spend bucketed into dev-meaningful units of work: cost per feature, fix, or task.";
    root.replaceChildren(lensSubtitle(subtitle), el("section", { class: "section" }, [el("h2", {}, ["Units of work"]), skelRows(4)]));
    let rep;
    try {
    ...
    if (declared.length === 0) {
        root.replaceChildren(lensSubtitle(subtitle), el("section", { class: "section" }, [
            el("h2", {}, ["Units of work"]),
            emptyState("No units of work configured", "Add [[unit]] entries to tare.toml, each with a name and a match rule (sessions, commits, or a run-id prefix). Then this view shows cost per task, PR, or feature.", { noAction: true }),
        ]));
        return;
    }
