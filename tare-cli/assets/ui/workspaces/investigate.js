...
export const MODES = ["runs", "sessions", "templates", "steps", "time"];
const MODE_LABEL = {
    runs: "Runs",
    sessions: "Sessions",
    templates: "Templates",
...
const SEARCH_FIELDS = ["id", "label", "hash", "tag", "note", "model", "config"];
...
export function baseCohort(query) {
    const filters = [];
    if (query.model)
        filters.push({ op: "eq", dimension: "model", value: query.model });
    if (query.provider)
        filters.push({ op: "eq", dimension: "provider", value: query.provider });
    if (query.session)
        filters.push({ op: "eq", dimension: "session", value: query.session });
    if (query.template)
        filters.push({ op: "eq", dimension: "template", value: query.template });
    if (query.tag)
        filters.push({ op: "tag", value: query.tag });
    if (query.minspend && Number.isFinite(Number(query.minspend))) {
        filters.push({ op: "gte_micros", value: Math.round(Number(query.minspend) * 1000000) });
    }
    return {
...
export function selectionWithFacet(cohort, dimension, value) {
    const filters = cohort.filters.filter((filter) => !((filter.op === "eq" || filter.op === "in") && filter.dimension === dimension));
    if (value)
        filters.push({ op: "eq", dimension, value });
    return { ...cohort, filters };
}
function selectedFacetValue(cohort, dimension) {
    if (!cohort)
        return null;
...
function withoutFirstFilter(filters, target) {
...
const LEGACY_FILTER_KEYS = ["model", "provider", "session", "template", "tag", "minspend"];
function legacyQueryFilters(query) {
    if (query.sel || !LEGACY_FILTER_KEYS.some((key) => Boolean(query[key])))
        return [];
    return baseCohort(query).filters;
}
function filterOwner(filter) {
    if (filter.op === "eq" || filter.op === "in")
        return `dimension:${filter.dimension}`;
    if (filter.op === "tag")
        return "tag";
    if (filter.op === "gte_micros")
        return "gte_micros";
    if (filter.op === "lte_micros")
        return "lte_micros";
    if (filter.op === "quality_range")
        return "quality_range";
    if (filter.op === "run_ids")
        return "run_ids";
    return "step_refs";
}
function applyLegacyFilters(cohort, filters) {
...
function analysisStateLink(nextState, query, fallbackHref, label, client, context, onSaveError) {
    if (!context)
...
export async function loadResults(client, mode, cohort, query) {
    const withScope = (extra) => routeHash("investigate", undefined, { ...query, ...extra });
...
function modeSelector(active, query) {
    const bar = el("div", {
        class: "inv-modes",
        role: "tablist",
...
function filterTokens(cohort, scope, selection, query, client, context, onSaveError) {
    ...
    for (const f of cohort.filters) {
        ...
        if (context) {
            ...
            let nextSelection = selection
                ...
                : null;
            if (nextSelection && sameCohort(nextSelection, nextScope))
                nextSelection = null;
            const nextState = {
                ...current,
            ...
            const navigation = analysisStateLink(nextState, query, fallbackHref, `Investigate without ${label}`, client, context, onSaveError);
...
function resultOverview(rows, mode) {
...
function quickFacetGroup(spec, rows, query, scope, selection, client, context, onSaveError) {
    const ranked = rows
    ...
    const navigationFor = (nextSelection, fallbackHref, label) => {
        if (!context)
            return { href: fallbackHref };
        const nextState = {
    ...
    if (activeValue) {
        ...
        const navigation = navigationFor(nextSelection, routeHash("investigate", undefined, { ...query, [spec.queryKey]: "", sel: "" }), `Investigate without ${spec.title.toLowerCase()} filter`);
    ...
    for (const row of ranked) {
        ...
        const navigation = navigationFor(nextSelection, routeHash("investigate", undefined, { ...query, [spec.queryKey]: row.value, sel: "", investigation: "" }), `Investigate Selection A · ${spec.dimension} ${row.value}`);
...
async function populateQuickFacets(host, client, scope, selection, query, context) {
    ...
    const groups = await Promise.all(QUICK_FACETS.map(async (spec) => {
        try {
            ...
            return { spec, rows: [], failed: true };
        }
    }));
    const content = groups
        .map(({ spec, rows }) => quickFacetGroup(spec, rows, query, scope, selection, client, context, onSaveError))
        .filter((node) => node !== null);
    host.replaceChildren(el("h2", { class: "subhead sub", text: selection ? "Refine Selection A" : "Build Selection A" }), ...content);
    if (content.length === 0) {
...
function resultTable(rows, mode) {
...
function analysisContext(scope, selection, baseline) {
    const active = selection ?? scope;
...
export async function renderInvestigate(root, client, route, context) {
    ...
    const advancedLens = query.view === "units"
    ...
    if (advancedLens) {
        ...
        root.replaceChildren(el("section", { class: "investigate-advanced", "aria-label": advancedLens.label }, [
        ...
        try {
            ...
            host.replaceChildren(errorNode(`Couldn't load ${advancedLens.label}.`, error, {
                actions: [
    ...
    const routeMode = query.mode === "timeline"
        ...
        : query.entity === "run"
            ...
            : query.entity === "step"
                ? "steps"
                : query.entity;
    const mode = MODES.includes(routeMode ?? "") ? routeMode : "runs";
    ...
    if (legacyFilters.length > 0 && context) {
        ...
        if (!selection || !sameCohort(selection, promoted))
            context.analysis.setSelection(promoted);
        selection = promoted;
    }
    ...
    const tokens = filterTokens(cohort, scope, selection, query, client, context, (error) => {
        const detail = error instanceof Error && error.message ? ` ${error.message}` : "";
        filterActionStatus.textContent = `Couldn't update these filters.${detail}`;
    });
    if (tokens)
        entityPane.appendChild(tokens);
    ...
    void (async () => {
        try {
            ...
            inspectorHost.replaceChildren(await renderInspector(client, cohort, baseline, query, {
    ...
    if (mode === "time") {
        resultOverviewHost.replaceChildren(el("section", { class: "inv-result-overview", "aria-labelledby": "inv-result-overview-h" }, [
        ...
        try {
            ...
            await renderInvestigateTimeline(resultHost, client, scope, selection, shared?.baseline ?? null, context, query);
        }
        catch (e) {
            resultHost.replaceChildren(errorNode("The Investigate timeline couldn't load.", e, {
                actions: [
                    { label: "Retry", primary: true, run: () => renderInvestigate(root, client, route, context) },
    ...
    try {
        ...
        resultOverviewHost.replaceChildren(el("p", { class: "caption sub", text: "Result summary is unavailable until the scope loads." }));
        resultHost.replaceChildren(errorNode("Investigate results couldn't load for this scope.", e, {
            actions: [
                { label: "Retry", primary: true, run: () => renderInvestigate(root, client, route, context) },
    ...
    const renderRows = (rs) => {
        ...
        resultHost.replaceChildren(rs.length === 0 ? el("p", { class: "caption sub", text: "No entities match this scope." }) : resultTable(rs, mode));
    };
    renderRows(rows);
    void populateQuickFacets(quickFacetHost, client, scope, selection, query, context);
        ...
        searchTimer = setTimeout(() => {
            void (async () => {
                ...
                try {
                    ...
                    const matched = new Set(res.data.entities.map((e) => (e.step_ordinal != null ? `${e.run_id}#${e.step_ordinal}` : e.run_id)));
