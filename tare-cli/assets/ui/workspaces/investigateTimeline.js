...
import { baselineLabel, priorWindowBaseline } from "../analysis/state.js";
...
export const TIMELINE_GROUPS = ["total", "provider", "model", "cause"];
...
function localCalendarDate(instant, timezone) {
    try {
        ...
        const part = (kind) => parts.find((candidate) => candidate.type === kind)?.value;
...
function actionCalendarDate(action, timezone) {
...
export function quarantineIncompleteDays(days, timezone, now = new Date()) {
...
function calendarDays(from, to) {
    ...
    if (days.length > 366)
        throw new TimelineCapabilityError("Timeline ranges over one year must be narrowed before rendering.");
    return days;
}
function sameTimelineCohort(a, b) {
    return canonicalCohortJson({ ...a, from: null, to: null }) === canonicalCohortJson({ ...b, from: null, to: null });
}
function buildSeries(observed, comparison, observedDays, comparisonDays, normalization, metric) {
    ...
    const values = (series, peers, days) => {
        if (!series) {
            return days.map((day) => {
                if (normalization === "absolute" && metric !== "cache_hit_rate")
                    return 0;
                if (normalization === "per_run" || metric === "cache_hit_rate")
                    return null;
    ...
    const total = (row) => row.reduce((sum, value) => sum + (value ?? 0), 0);
    return [...keys]
        .map((key) => ({
        key,
...
async function defaultExtent(client, scope) {
    if (scope.from && scope.to)
        return calendarDays(scope.from, scope.to);
...
export async function loadInvestigateTimeline(client, scope, baselineSpec, group, now = new Date()) {
    ...
    if (split.complete.length === 0) {
        throw new TimelineCapabilityError(split.quarantined.length > 0
    ...
    if (baselineSpec && (!baselineSpec.cohort.from || !baselineSpec.cohort.to)) {
        throw new TimelineCapabilityError(`${baselineLabel(baselineSpec.kind)} Baseline B has no bounded calendar window. Update or clear it before timeline comparison; it is not silently rewritten.`);
    }
    ...
    const interventionAnnotations = actions
    ...
    const configAnnotations = observedResponse.data.config_events.map((event) => ({
        date: event.day,
        kind: "config_change",
    ...
    return {
        ...
        series: buildSeries(observedResponse.data.series, baselineResponse.data.series, observedResponse.data.days, baselineResponse.data.days, scope.normalization, scope.metric),
        annotations: [...interventionAnnotations, ...configAnnotations].sort((a, b) => a.date.localeCompare(b.date) || a.kind.localeCompare(b.kind) || a.label.localeCompare(b.label)),
...
function timelineSvg(model) {
    ...
    const x = (index) => left + (n <= 1 ? plotW / 2 : (index / (n - 1)) * plotW);
    const y = (value) => top + plotH - (Math.max(0, value) / denom) * plotH;
    const path = (values) => {
        let open = false;
    ...
    out.push(`<text x="${left}" y="${height - 10}" fill="var(--muted)" font-size="10">${esc(model.days[0])}</text>`, `<text x="${width - right}" y="${height - 10}" fill="var(--muted)" font-size="10" text-anchor="end">${esc(model.days[model.days.length - 1])}</text>`, "</svg>");
    return out.join("");
}
function timelineHash(hash, group) {
    const parsed = parseHash(hash);
    return routePath(parsed.segments, { ...(parsed.query ?? {}), group, mode: "timeline" });
}
function selectControl(className, label, value, choices) {
    const select = el("select", { class: className, "aria-label": label }, choices.map((choice) => el("option", {
        value: choice.value,
        text: choice.label,
...
function stateWithMetric(context, metric, normalization) {
    const current = context.analysis.get();
    const update = (cohort) => ({ ...cohort, metric, normalization });
    return {
        ...current,
...
function navigateTimelineState(context, next, group) {
...
export async function renderInvestigateTimeline(host, client, scope, selection, baseline, context, query) {
    const requestedGroup = query.group;
    const group = requestedGroup && TIMELINE_GROUPS.includes(requestedGroup) ? requestedGroup : "total";
    ...
    if (context) {
        metric.select.addEventListener("change", () => {
            const nextMetric = metric.select.value;
            const nextNormalization = nextMetric === "cache_hit_rate"
                ? "absolute"
                : normalization.select.value;
        ...
        normalization.select.addEventListener("change", () => {
            const nextNormalization = normalization.select.value;
            navigateTimelineState(context, stateWithMetric(context, metric.select.value, nextNormalization), group);
        });
        grouping.select.addEventListener("change", () => {
            const nextGroup = grouping.select.value;
            const serialized = serializeAnalysisHash(["investigate"], context.analysis.get());
            if (serialized.kind === "inline")
                window.location.hash = timelineHash(serialized.hash, nextGroup);
        });
    }
    ...
    if (model.annotations.length > 0) {
        shell.appendChild(el("ul", { class: "inv-timeline-annotations", "aria-label": "Timeline annotations" }, model.annotations.map((annotation) => el("li", { text: `${annotation.date} · ${annotation.label}` }))));
    }
    ...
    if (selection?.from && model.days.includes(selection.from))
        start.value = String(model.days.indexOf(selection.from));
    if (selection?.to && model.days.includes(selection.to))
        end.value = String(model.days.indexOf(selection.to));
    const output = el("output", { class: "inv-brush-output caption", "aria-live": "polite" });
    const apply = el("button", { class: "btn primary inv-brush-apply", type: "button", text: "Apply brushed range" });
    ...
    apply.addEventListener("click", () => {
        ...
        const selected = { ...active, from: range.from, to: range.to };
