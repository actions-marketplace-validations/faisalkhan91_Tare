...
export function flameWeightOf(node, weight) {
...
function fillFor(node, theme) {
    if (node.cache_class)
        return theme.classColor[node.cache_class] ?? theme.defaultFill;
    return theme.defaultFill;
}
function renderNode(node, x, w, depth, out, theme, counter, weight) {
    ...
    out.push(`<g><rect x="${x}" y="${y}" width="${w}" height="${h}" fill="${fillFor(node, theme)}" stroke="${theme.stroke}" stroke-width="1"${frameAttr}/><title>${esc(title)}</title>`);
    ...
    if (maxChars >= 3) {
        ...
        out.push(`<text x="${x + 4}" y="${y + 14}" font-family="monospace" font-size="11" fill="${theme.labelFill}">${esc(label)}</text>`);
    }
    out.push("</g>");
...
function renderLegend(y, out, theme) {
    let x = PAD;
    for (const c of LEGEND_ORDER) {
        out.push(`<rect x="${x}" y="${y}" width="12" height="12" fill="${theme.classColor[c]}"/><text x="${x + 16}" y="${y + 10}" font-family="monospace" font-size="10" fill="${theme.legendFill}">${esc(c)}</text>`);
...
function renderFlame(model, theme, weight) {
    ...
    if (flameWeightOf(model.root, weight) === 0) {
        ...
        const msg = weight === "cost" && model.root.tokens > 0
...
export function renderSvgThemed(model, weight = "cost") {
