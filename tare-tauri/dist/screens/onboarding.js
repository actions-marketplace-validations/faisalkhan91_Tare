...
import { setOnboarded, onboardStep, setOnboardStep, } from "../ui/prefs.js";
...
const TROUBLESHOOT_AFTER_MS = 30000;
export async function renderOnboarding(root, client) {
    ...
    const hasCapturedSpend = async () => {
        try {
            if ((await client.today()).total_micros > 0)
                return true;
    ...
    const done = async (destination = "pulse") => {
        setOnboarded(true);
        ...
        try {
            if (!(await hasCapturedSpend()))
                latestRunId = await client.seedDemo();
            else if (!latestRunId)
                latestRunId = await lastRunId();
        }
        catch {
    ...
    const stepper = () => el("div", { class: "onboard-steps" }, 
    // "See spend" (verb-goal), NOT "Profile" — which collided with the Privacy profile control in
    // step 3 (one word, two unrelated meanings).
    ["Capture", "See spend", "Tune"].map((label, i) => el("span", { class: `onboard-step${i === step ? " active" : ""}${i < step ? " done" : ""}`, text: `${i + 1}. ${label}` })));
    // ---- Step 1: Capture ----
    function capture() {
        ...
        if (client.canControlProxy?.()) {
            ...
            const startBtn = el("button", { class: "btn", text: "Start capture service" });
    ...
    function tune() {
        const maxSpend = el("input", { type: "number", value: capUsd });
        const prof = el("select", {}, PROFILES.map((p) => el("option", { value: p, text: humanizeKey(p), ...(p === profileName ? { selected: "" } : {}) })));
        ...
        const finish = async () => {
            ...
            if (canSave) {
                ...
                const toml = (cap === undefined ? "[budget]\n" : `[budget]\nmax_spend_usd = ${cap}\n`) + `\n[privacy]\nprofile = "${prof.value}"\n`;
                out.replaceChildren(el("p", { class: "caption", text: "Save this as tare.toml next to where you run Tare:" }), el("pre", { class: "explain", text: toml }), el("button", { class: "btn", text: "Done → Pulse", onClick: () => void done() }));
            }
        };
        return el("section", { class: "section onboard" }, [
            stepper(),
