...
function spendTone(micros, maxSpend) {
    const share = maxSpend > 0 ? micros / maxSpend : 0;
    return share >= 0.66 ? "cost-high" : share >= 0.33 ? "cost-warn" : "cost-ok";
}
const dollarCell = (m) => el("span", { class: "dollars", text: fmtUsd(m), title: toDollarString(m) });
export async function renderCorrelate(root, client) {
    const subtitle = "Spend projected onto the config knobs that drive it: which settings cost you.";
    root.replaceChildren(lensSubtitle(subtitle), el("section", { class: "section" }, [el("h2", {}, ["Correlate"]), skelRows(6)]));
    let rows;
    let pricingVersion = "";
    try {
    ...
    const plotSection = rows.length >= 2
        ? el("section", { class: "section" }, [
            ...
            parcoords(AXES, lines, {
                ...
                axisFormat: (i, v) => i === 3 ? fmtTokens(v) : i === 4 ? fmtUsd(v) : i === 5 ? String(Math.round(v)) : AXES[i] === "latency" ? fmtDuration(v) : "",
            }),
    ...
    root.replaceChildren(lensSubtitle(subtitle), plotSection, el("section", { class: "section" }, [
