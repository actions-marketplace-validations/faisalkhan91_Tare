...
function trustHref(route, pricing = false) {
    const query = { ...(route.query ?? {}), sheet: "trust" };
    if (pricing) {
        ...
        if (query.workspace_view)
            query.view = query.workspace_view;
        else
            delete query.view;
...
function trustNav(route, pricing) {
    const summary = el("a", {
...
function pricingOverview(info, confidence) {
...
function coverageSection(coverage) {
    const status = coverage.status === "green"
        ? "Capture healthy"
    ...
    const tone = coverage.status === "green"
        ? "cost-ok"
        : coverage.status === "amber"
            ? "cost-warn"
            : coverage.status === "red"
    ...
    if (coverage.sources.length === 0) {
        ...
        section.appendChild(el("table", { class: "data trust-source-table" }, [
            el("thead", {}, [
                el("tr", {}, [
            ...
            el("tbody", {}, coverage.sources.map((source) => el("tr", {}, [
    ...
    if (coverage.blind_sources.length > 0) {
        section.appendChild(el("p", { class: "unpriced" }, [
...
function fidelityLine(provenance) {
...
function provenanceSection(response) {
    const { data, provenance } = response;
    const coverage = provenance.coverage_status === "unknown"
...
function reconciliationSection(reconciliation) {
    ...
    if (reconciliation.rows.length === 0) {
        section.appendChild(el("p", { class: "sub", text: "A vendor metric was captured, but there are no comparable priced rows. No reconciliation percentage is claimed." }));
        return section;
    }
    ...
    section.appendChild(el("table", { class: "data trust-reconciliation-table" }, [
        el("thead", {}, [
            el("tr", {}, [
        ...
        el("tbody", {}, reconciliation.rows.map((row) => el("tr", {}, [
...
export async function renderTrust(root, client, route, context) {
    ...
    const [pricingResult, confidenceResult, coverageResult, provenanceResult, reconciliationResult] = await Promise.allSettled([
        client.pricing(),
        client.confidence(),
        client.coverage(),
    ...
    if (pricing && confidence)
        children.push(pricingOverview(pricing, confidence));
    else
        children.push(errorNode("Couldn't load pricing freshness. Dollar estimates cannot be verified right now.", pricingResult.status === "rejected" ? pricingResult.reason : confidenceResult.status === "rejected" ? confidenceResult.reason : undefined));
    if (coverage)
        children.push(coverageSection(coverage));
    else
        children.push(errorNode("Couldn't load local capture health. Completeness remains unknown.", coverageResult.status === "rejected" ? coverageResult.reason : undefined));
    if (provenance)
        children.push(provenanceSection(provenance));
    else
        children.push(errorNode("Couldn't resolve provenance for the current scope. No completeness claim is available.", provenanceResult.status === "rejected" ? provenanceResult.reason : undefined));
    if (reconciliation)
        children.push(reconciliationSection(reconciliation));
    else
        children.push(errorNode("Couldn't load the vendor reconciliation cross-check. Tare's estimate remains separate.", reconciliationResult.status === "rejected" ? reconciliationResult.reason : undefined));
