...
export async function renderReceiptVerifier(root, client, options = {}) {
    ...
    if (showLens) {
        frag.appendChild(lensSubtitle("The bundled price book and a line-item cost receipt for any run. Every figure is estimated, never billed."));
    }
    // Pricing freshness.
    ...
    if (runsError !== null) {
        ...
        section.appendChild(errorNode("Couldn't load runs. This is a load failure, not an empty history. Check that the capture service is running.", runsError, {
            actions: [
                {
                    label: "Retry",
                    primary: true,
    ...
    if (!runs || runs.length === 0) {
        section.appendChild(emptyState("No runs to attest yet", "Capture a run, then create a recomputation receipt to share or verify its cost."));
        frag.appendChild(section);
        root.replaceChildren(frag);
        return;
    }
    const sel = el("select", {}, runs.map((r) => el("option", { value: r, text: r })));
    if (initialRunId && runs.includes(initialRunId))
        sel.value = initialRunId;
    const priv = el("input", { type: "checkbox" });
    const btn = el("button", { class: "btn", text: "Attest + verify" });
    const out = el("div");
    section.appendChild(el("div", { class: "diff-controls" }, [
        el("label", {}, ["Run ", sel]),
    ...
    async function run() {
        ...
        try {
            ...
            out.replaceChildren(errorNode("Couldn't attest this run. You can retry or pick another run.", e, {
        ...
        out.replaceChildren(receiptStatement(v), el("p", { class: "caption", text: `Scope: ${v.scope}. Re-derived offline from the bundled pricing — you just verified the number; no network required.` }));
    }
