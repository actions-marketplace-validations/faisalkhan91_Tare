...
import { connectEnvFor, connectEnvShellFor, pricingFreshnessLine, captureRecommendation, otelEnv, otelEnvShell, providerPreset, PROVIDER_PRESETS, AGENT_RECIPES, agentRecipe, } from "../ui/connectEnv.js";
...
export function paintCapturePill(pill, status, mode, jsonl) {
    let tone;
    let text;
    ...
    if (mode !== undefined) {
        if (mode === "off") {
            ...
            if (otlpLive) {
                tone = "ok";
                text = `Capturing via OTel · Live event ${liveAge(status.age_seconds)}`;
            }
            else {
                tone = "warn";
            ...
            text = otlpLive
                ? `Capturing (${where}) · Live event ${liveAge(status.age_seconds)}`
                : `Capturing (${where})`;
        }
    }
    else if (!status || !status.listening) {
    ...
    pill.replaceChildren(icon("dot", { size: 8, class: `capture-dot capture-${tone}` }), el("span", { class: "capture-text", text }));
}
function copy(text) {
    try {
...
export async function renderConnect(root, client) {
    const canControl = client.canControlProxy?.() ?? false;
    let status = { running: false, port: 0, url: "" };
    try {
    ...
    const otelRows = otelEnv(otlpUrl).map(([k, v]) => el("tr", {}, [
    ...
    {
        const agentSel = el("select", { "aria-label": "Agent" }, AGENT_RECIPES.map((a) => el("option", { value: a.id, text: a.label })));
    ...
    if (cov && cov.status !== "none") {
        ...
        if (cov.blind_sources.length > 0) {
            block.appendChild(el("p", { class: "error", text: `No cost data: ${cov.blind_sources.join(", ")} sent heartbeats, but no cost steps.` }));
        }
    ...
    if (canControl) {
        ...
        rows.push(proxyErr);
        frag.appendChild(el("section", { class: "section local-service" }, [el("h2", {}, ["Capture service"]), ...rows]));
    }
    ...
    const renderProviderEnv = () => {
        ...
        recoBanner.replaceChildren(el("span", { class: "reco-tag", text: "Recommended" }), el("span", {}, [
        ...
        if (pairs.length === 0) {
            ...
            envBody.replaceChildren(el("tr", {}, [el("td", { class: "cause", text: "No override" }), el("td", { text: "No base-URL override for this provider; use the recommended OTel path above." }), el("td", {})]));
            pasteBlock.textContent = "";
            copyAll.replaceChildren();
            return;
        }
        envBody.replaceChildren(...pairs.map(([k, v]) => el("tr", {}, [
            el("td", { class: "cause", text: k }),
            el("td", { class: "num", text: v }),
            el("td", {}, [copyBtn(v)]),
        ])));
    ...
    }, PROVIDER_PRESETS.map((p) => el("option", { value: p.id, text: p.label, ...(p.id === selected ? { selected: "" } : {}) })));
    renderProviderEnv();
    frag.appendChild(el("section", { class: "section" }, [
