...
export function createStatusBar(mk) {
    const dot = mk("span", {
        class: "status-dot",
        role: "img",
    ...
    const el = mk("footer", { class: "statusbar", "aria-label": "Workspace status" }, [conn, scope, mk("span", { class: "spacer" }), warning, updated]);
    return {
        el,
        setConnection(state) {
            dot.classList.toggle("ok", state === true);
            const label = state === true
        ...
        setScope(s) {
            ...
            if (s.count !== undefined)
                parts.push(`${s.count.toLocaleString()} ${s.count === 1 ? "run" : "runs"}`);
            if (s.spendMicros !== undefined)
                parts.push(fmtUsd(s.spendMicros));
