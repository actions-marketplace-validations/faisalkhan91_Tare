...
import { flameWeightOf } from "../svg.js";
/// The single heaviest child by `weight`; ties break by name for deterministic output.
function heaviestChild(children, weight) {
    return children.reduce((a, b) => flameWeightOf(b, weight) > flameWeightOf(a, weight) ||
...
/// input untouched.
export function cullSubPixel(model, minWidthPx = 1, renderWidthPx = 960, weight = "tokens") {
    const total = flameWeightOf(model.root, weight);
    if (total <= 0 || minWidthPx <= 0 || renderWidthPx <= 0)
        return model;
