...
export function setOnboardStep(n) {
    try {
...
export function providerPref() {
    return store()?.getItem(PK) ?? "anthropic";
}
export function setProviderPref(id) {
    try {
        store()?.setItem(PK, id);
    }
    catch {
...
export function setBaselineRun(id) {
    try {
        if (id)
            store()?.setItem(BASEK, id);
        else
            store()?.removeItem(BASEK);
    }
    catch {
        /* ignore */
    }
}
export function showBaselineDelta() {
    return store()?.getItem(BDLTK) !== "0"; // default on
}
export function setShowBaselineDelta(v) {
    try {
...
export function recordOpenedRun(id) {
 ...
 */
export function forgetRunReferences(id) {
...
export function reconcileRunReferences(validRunIds) {
...
export function rangePref() {
    const raw = store()?.getItem(RNG);
    if (!raw)
        return { key: "30d" };
    try {
        const v = JSON.parse(raw);
        const key = v.key === "7d" || v.key === "90d" || v.key === "custom" ? v.key : "30d";
...
export function setRangePref(p) {
    try {
...
export function defaultScreen() {
    ...
    if (v === "live" || v === "overview")
        return "pulse";
    if (v === "runs" || v === "sessions" || v === "trends")
        return "investigate";
    if (v === "experiments")
        return "optimize";
    return v && DEFAULT_SCREENS.includes(v) ? v : "pulse";
}
export function setDefaultScreen(s) {
    try {
        if (DEFAULT_SCREENS.includes(s))
            store()?.setItem(DS, s);
    }
    catch {
        /* ignore */
...
export function applyDensity(d, doc = document) {
