...
export function filterCommands(cmds, query) {
    return cmds.filter((c) => fuzzyMatch(query, [c.title, c.subtitle, c.section, c.keywords].filter(Boolean).join(" ")));
}
/// Open the palette over `commands`. Returns the overlay element (removed on close/activate).
export function openPalette(commands, doc = document) {
...
export function installPaletteHotkey(open, win = window, os = osClass(typeof navigator !== "undefined" ? navigator.userAgent : "")) {
    const handler = (e) => {
