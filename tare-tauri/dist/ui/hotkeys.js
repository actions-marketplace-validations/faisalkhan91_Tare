...
export function chordState(bindings, buffer) {
    if (buffer.length === 0)
        return { status: "none" };
    const chord = buffer.join(" ");
    const exact = bindings.find((b) => b.seq === chord);
    if (exact)
        return { status: "exact", binding: exact };
...
export function installHotkeys(bindings, win = window, opts = {}) {
    ...
    const handler = (e) => {
        if (e.metaKey || e.ctrlKey || e.altKey)
            return; // leave modifier shortcuts (⌘K) alone
