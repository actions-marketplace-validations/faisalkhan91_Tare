...
function cohortWindow(cohort) {
    if (cohort.from && cohort.to) {
        return cohort.from === cohort.to ? cohort.from : `${cohort.from}–${cohort.to}`;
    }
    if (cohort.from)
        return `From ${cohort.from}; no end date supplied`;
    if (cohort.to)
        return `Through ${cohort.to}; no start date supplied`;
...
function rangeText(opportunity, snapshot) {
...
function evidenceFacts(options) {
    ...
    const share = currentSpend != null && currentSpend > 0
...
function evidenceReferences(opportunity, drillAttributes) {
    ...
    if (runRefs.length > 0) {
        runDetails.appendChild(el("ul", { class: "optimize-reference-list" }, runRefs.map((runId) => el("li", {}, [el("code", { text: runId })]))));
    }
    else {
    ...
    if (stepRefs.length > 0) {
        stepDetails.appendChild(el("ul", { class: "optimize-reference-list" }, stepRefs.map((ref) => el("li", {}, [el("code", { text: `${ref.run_id}#${ref.step_ordinal}` })]))));
    }
    else {
...
function confidenceSection(opportunity) {
    const section = el("section", { class: "optimize-evidence-section" }, [
    ...
    if (opportunity.assumptions.length === 0) {
        ...
        section.appendChild(el("ul", { class: "optimize-assumption-list" }, opportunity.assumptions.map((assumption) => el("li", { text: assumption }))));
    }
    return section;
}
function exactCohortSection(cohort) {
    const section = el("section", { class: "optimize-evidence-section" }, [
    ...
    if (cohort.filters.length === 0) {
        ...
        section.appendChild(el("ul", { class: "optimize-cohort-filter-list" }, cohort.filters.map((filter) => el("li", { text: filterText(filter) }))));
    }
    return section;
}
function actionSnapshotSection(snapshot) {
...
function provenanceSection(resolved) {
    const section = el("section", { class: "optimize-evidence-section" }, [
    ...
    section.append(el("dl", { class: "optimize-evidence-pairs" }, [
    ...
    if (provenance.assumptions.length > 0) {
        section.append(el("h6", { text: "Provenance assumptions" }), el("ul", { class: "optimize-assumption-list" }, provenance.assumptions.map((assumption) => el("li", { text: assumption }))));
    }
    return section;
}
export function renderOpportunityEvidence(options) {
