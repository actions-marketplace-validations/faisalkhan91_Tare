...
const MAX_STEPS = 8;
const sessionKey = (s) => `session:${s.source}/${s.session}`;
const stepKey = (s) => `step:${s.run_id}#${s.ordinal}`;
...
function wireSelect(row, key, section, state) {
    row.addEventListener("click", () => {
        state.selected = state.selected === key ? null : key;
        for (const r of Array.from(section.querySelectorAll(".pulse-now-row"))) {
...
export function reconcileFeed(section, sessions, steps, state) {
    const sessList = section.querySelector(".pulse-now-sessions");
    const stepList = section.querySelector(".pulse-now-steps");
    const reconcile = (list, rows, emptyText) => {
        if (rows.length === 0) {
            list.replaceChildren(el("li", { class: "pulse-now-empty caption sub", text: emptyText }));
            return;
        }
        const existing = new Map();
        for (const li of Array.from(list.querySelectorAll("li[data-key]"))) {
        ...
        for (const r of rows) {
            ...
            const a = li.querySelector(".pulse-now-row");
    ...
    reconcile(sessList, sessions.slice(0, MAX_SESSIONS).map((s) => ({
        key: sessionKey(s),
    ...
    reconcile(stepList, steps.slice(0, MAX_STEPS).map((s) => ({
        key: stepKey(s),
...
export async function pollNowFeed(section, client, state) {
    try {
...
export function renderNowFeed(client, opts = {}) {
