...
const DISTINGUISH_DIMS = ["model", "provider", "tool", "session", "template"];
...
export function rankFactors(dimension, rows, topN = 3) {
    return rows
...
export function representatives(entities) {
    ...
    const mostAnomalous = entities.reduce((best, e) => (Math.abs(e.matched_micros - mean) > Math.abs(best.matched_micros - mean) ? e : best), entities[0]);
    return { largest: sorted[0], mostAnomalous, median: sorted[Math.floor(sorted.length / 2)] };
}
function runLink(runId, query) {
    return el("a", {
        class: "insp-run",
...
export async function renderInspector(client, selection, baseline, query = {}, options = {}) {
    ...
    if (!comparisonReady) {
        aside.appendChild(el("section", { class: "insp-start" }, [
        ...
        const facetResults = await Promise.all(DISTINGUISH_DIMS.map((d) => client.facetCohort({ selection, baseline, dimension: d }).then((r) => rankFactors(d, r.data.rows)).catch(() => [])));
        ...
        if (factors.length === 0) {
            ...
            for (const f of factors) {
                list.appendChild(el("li", { class: "insp-factor" }, [
        ...
        try {
            ...
            for (const w of d.compatibility_warnings)
                decomp.appendChild(el("p", { class: "caption sub", text: w }));
    ...
    if (reps) {
        repsSec.appendChild(el("ul", { class: "insp-rep-list" }, [
...
function provenanceBlock(p) {
    ...
    for (const a of p.assumptions)
        sec.appendChild(el("p", { class: "caption sub", text: a }));
    return sec;
}
