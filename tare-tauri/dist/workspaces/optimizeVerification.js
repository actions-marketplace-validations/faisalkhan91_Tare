...
export function verificationWindows(action) {
...
export function verificationLifecycleState(result) {
    if (!result.complete)
        return "verifying";
...
export function verificationFormulaMatches(action, result) {
    let computed;
    if (action.baseline) {
        if (result.baseline_before_micros == null ||
...
export function verificationPresentationState(action, result) {
    return verificationFormulaMatches(action, result)
        ? verificationLifecycleState(result)
        : "applied";
}
function matchRuleText(action) {
...
function outcomeText(action) {
    ...
    if (!denominator)
        return "None stored; no causal outcome-denominator claim";
    return denominator.kind === "work_unit"
        ? `Work unit · ${denominator.name}`
        : `Metered · ${humanizeKey(denominator.metered)}`;
}
function countText(matched, unmatched, aggregateOnly) {
    if (matched == null || unmatched == null)
        return "Match/exclusion counts unavailable";
    if (aggregateOnly) {
...
function windowCard(title, period, micros, matched, unmatched, aggregateOnly) {
...
function formulaDetails(action, result) {
...
function statusStatement(action, result, state, formulaMatches) {
...
export function renderSavingsVerification(action, result) {
    ...
    const panel = el("section", {
        ...
        el("div", { class: "optimize-verification-windows" }, [
            windowCard("Selection · before", `${windows.beforeFrom}–${windows.beforeTo}`, result.selection_before_micros, result.matched_before, result.unmatched_before, aggregateOnly),
            windowCard("Selection · after", `${windows.afterFrom}–${windows.afterTo}`, result.selection_after_micros, result.matched_after, result.unmatched_after, aggregateOnly),
        ]),
    ]);
    if (action.baseline) {
        panel.appendChild(el("div", { class: "optimize-verification-baseline-head" }, [
        ...
        panel.appendChild(el("div", { class: "optimize-verification-windows" }, [
            windowCard("Baseline · before", `${windows.beforeFrom}–${windows.beforeTo}`, result.baseline_before_micros, result.baseline_matched_before, result.baseline_unmatched_before, aggregateOnly),
            windowCard("Baseline · after", `${windows.afterFrom}–${windows.afterTo}`, result.baseline_after_micros, result.baseline_matched_after, result.baseline_unmatched_after, aggregateOnly),
        ]));
    }
    else {
