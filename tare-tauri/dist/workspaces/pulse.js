...
function requestedRange(route) {
    const value = route.query?.range;
    return PULSE_RANGES.some((range) => range.value === value)
        ? value
        : undefined;
}
function periodScopeLabel(b) {
    if (b.period === "day")
        return "Today · captured spend";
...
function pulseRangeSelector(b, route) {
...
export function pulseBeamModel(savings, unpricedSharePct) {
    ...
    if (share > 0)
        model.gap = { label: "Unpriced usage", share };
    return model;
}
...
function fmtUsdApprox(micros) {
    const safe = nonnegativeFinite(micros);
    return safe < 1000000 ? fmtUsd(safe) : usdWhole.format(safe / 1000000);
}
function fmtAxisUsd(micros) {
    const dollars = nonnegativeFinite(micros) / 1000000;
    if (dollars === 0)
        return "$0";
    if (dollars >= 1000000)
        return `$${Number((dollars / 1000000).toFixed(1))}m`;
    if (dollars >= 1000)
        return `$${Number((dollars / 1000).toFixed(1))}k`;
    if (dollars >= 1)
...
function civilDateAt(start, offset) {
...
function captureIsIncomplete(cov) {
...
function niceMoneyCeiling(value) {
...
function spreadLabelYs(labels, minY, maxY, gap) {
    if (labels.length === 0)
        return new Map();
...
function forecastLineSvgVariant(b, variant) {
    ...
    const x = (d) => LEFT + (d / days) * (PLOT_RIGHT - LEFT);
    ...
    for (const [index, tick] of ticks.entries()) {
        const tickY = y(tick);
        out.push(`<line class="pulse-gridline" x1="${LEFT}" y1="${tickY.toFixed(1)}" x2="${PLOT_RIGHT}" y2="${tickY.toFixed(1)}" stroke="var(--border-strong)" stroke-opacity="${index === ticks.length - 1 ? "0.52" : "0.22"}"/>`);
        out.push(`<text class="pulse-y-tick" x="${LEFT - 8}" y="${(tickY + 3.5).toFixed(1)}" text-anchor="end" font-size="10.5" fill="var(--muted)">${fmtAxisUsd(tick)}</text>`);
    }
    if (cap > 0) {
        out.push(`<line class="pulse-cap" x1="${x(0).toFixed(1)}" y1="${y(0).toFixed(1)}" x2="${x(days).toFixed(1)}" y2="${y(cap).toFixed(1)}" stroke="var(--border-strong)" stroke-width="1.75" stroke-dasharray="2 3"/>`);
    }
    out.push(`<line class="pulse-today" x1="${currentX.toFixed(1)}" y1="${TOP}" x2="${currentX.toFixed(1)}" y2="${BASE}" stroke="var(--border-strong)" stroke-opacity="0.7" stroke-dasharray="2 4"/>`);
    out.push(`<text class="pulse-today-label" x="${(currentX + (currentOnRight ? -6 : 6)).toFixed(1)}" y="${TOP + 13}" text-anchor="${currentOnRight ? "end" : "start"}" font-size="10.5" fill="var(--muted)">Today · ${escapeSvg(asOfLabel)}</text>`);
    if (remainingDays > 0 && bandHigh > bandLow) {
        out.push(`<polygon class="pulse-band" points="${pt(elapsed, observedEnd)} ${pt(days, bandHigh)} ${pt(days, bandLow)}" fill="var(--data-ink)" fill-opacity="0.18"/>`, `<line class="pulse-band-edge" x1="${currentX.toFixed(1)}" y1="${currentY.toFixed(1)}" x2="${x(days).toFixed(1)}" y2="${y(bandHigh).toFixed(1)}" stroke="var(--data-ink)" stroke-opacity="0.72" stroke-width="1.5"/>`, `<line class="pulse-band-edge" x1="${currentX.toFixed(1)}" y1="${currentY.toFixed(1)}" x2="${x(days).toFixed(1)}" y2="${y(bandLow).toFixed(1)}" stroke="var(--data-ink)" stroke-opacity="0.72" stroke-width="1.5"/>`);
    }
    if (historyIsComplete && elapsed > 0) {
        ...
        out.push(`<path class="pulse-observed" d="${path.join(" ")}" fill="none" stroke="var(--data-ink)" stroke-width="2.75" stroke-linecap="round" stroke-linejoin="round"/>`);
    }
    else if (elapsed > 0) {
        out.push(`<text class="pulse-history-missing" x="${LEFT + 8}" y="${BASE - 10}" font-size="10.5" fill="var(--muted)">Daily history unavailable</text>`);
    }
    if (remainingDays > 0) {
        out.push(`<line class="pulse-forecast" x1="${currentX.toFixed(1)}" y1="${currentY.toFixed(1)}" x2="${x(days).toFixed(1)}" y2="${y(projected).toFixed(1)}" stroke="var(--data-ink)" stroke-opacity="0.82" stroke-width="2.5" stroke-linecap="round" stroke-dasharray="5 4"/>`);
    }
    out.push(`<circle class="pulse-current-point" cx="${currentX.toFixed(1)}" cy="${currentY.toFixed(1)}" r="4" fill="var(--data-ink)"/>`);
    if (remainingDays > 0) {
        out.push(`<circle class="pulse-forecast-point" cx="${x(days).toFixed(1)}" cy="${y(projected).toFixed(1)}" r="4" fill="var(--bg)" stroke="var(--data-ink)" stroke-width="2"/>`);
    }
    out.push(`<text class="pulse-current-value pulse-direct-label" x="${currentLabelX.toFixed(1)}" y="${currentLabelY.toFixed(1)}" text-anchor="${currentOnRight ? "end" : "start"}" font-size="10.5" font-weight="600" fill="var(--text)">Captured ${escapeSvg(fmtUsdApprox(observedEnd))}</text>`);
    const endpointLabels = remainingDays > 0 ? [
        {
            key: "typical",
    ...
    if (remainingDays > 0 && bandHigh > bandLow) {
        endpointLabels.push({ key: "high", text: compact ? `High ${fmtAxisUsd(bandHigh)}` : `High pace · ${fmtUsdApprox(bandHigh)}`, targetY: y(bandHigh), stroke: "var(--data-ink)", fill: "var(--text)" }, { key: "low", text: compact ? `Low ${fmtAxisUsd(bandLow)}` : `Low pace · ${fmtUsdApprox(bandLow)}`, targetY: y(bandLow), stroke: "var(--data-ink)", fill: "var(--text)" });
    }
    if (cap > 0) {
    ...
    for (const label of endpointLabels) {
        ...
        out.push(`<line class="pulse-label-leader" x1="${PLOT_RIGHT.toFixed(1)}" y1="${label.targetY.toFixed(1)}" x2="${endpointLeaderX.toFixed(1)}" y2="${(labelY - 3).toFixed(1)}" stroke="${label.stroke}" stroke-width="1"/>`, `<text class="pulse-direct-label pulse-label-${label.key}" x="${endpointTextX.toFixed(1)}" y="${labelY.toFixed(1)}" text-anchor="${endpointTextAnchor}" font-size="${compact ? "9.5" : "10.5"}" font-weight="600" fill="${label.fill}">${escapeSvg(label.text)}</text>`);
    }
    out.push(`<text x="${LEFT}" y="${H - 10}" font-size="10.5" fill="var(--muted)">${escapeSvg(startLabel)}</text>`);
    out.push(`<text x="${PLOT_RIGHT}" y="${H - 10}" text-anchor="end" font-size="10.5" fill="var(--muted)">${escapeSvg(endLabel)}</text>`);
    out.push(`<rect class="pulse-chart-hit-target" x="${LEFT}" y="${TOP}" width="${(PLOT_RIGHT - LEFT).toFixed(1)}" height="${(BASE - TOP).toFixed(1)}" fill="transparent"/>`, `<g class="pulse-inspector" aria-hidden="true">`, `<line class="pulse-inspector-line" x1="${LEFT}" y1="${TOP}" x2="${LEFT}" y2="${BASE}"/>`, `<circle class="pulse-inspector-point series-captured" data-inspector-series="captured" r="4"/>`, `<circle class="pulse-inspector-point series-typical" data-inspector-series="typical" r="4"/>`, `<circle class="pulse-inspector-point series-low" data-inspector-series="low" r="3.5"/>`, `<circle class="pulse-inspector-point series-high" data-inspector-series="high" r="3.5"/>`, `<circle class="pulse-inspector-point series-cap" data-inspector-series="cap" r="3.5"/>`, `</g>`);
    out.push(`</svg>`);
    return out.join("");
}
function forecastLineSvg(b) {
...
function wireForecastInteraction(host, b) {
    ...
    const valuesAt = (index) => {
        ...
        if (index <= elapsed) {
            if (historyIsComplete)
                values.captured = cumulative[index - 1] ?? 0;
            else if (index === elapsed)
                values.captured = spent;
        }
        ...
        if (b.cap_micros > 0)
            values.cap = Math.round(nonnegativeFinite(b.cap_micros) * index / days);
        return values;
    };
    const tooltipRow = (series, label, value) => el("div", { class: `pulse-tooltip-row series-${series}` }, [
        el("dt", {}, [
    ...
    const show = (svg, requestedIndex) => {
        ...
        const y = (value) => plotBase - (value / domainTop) * (plotBase - plotTop);
        const group = svg.querySelector(".pulse-inspector");
        const line = group?.querySelector(".pulse-inspector-line");
        if (line) {
            line.setAttribute("x1", x.toFixed(1));
            line.setAttribute("x2", x.toFixed(1));
        }
        const values = valuesAt(index);
        for (const series of ["captured", "typical", "low", "high", "cap"]) {
            const point = group?.querySelector(`[data-inspector-series="${series}"]`);
        ...
        if (values.captured !== undefined) {
            ...
            if (historyIsComplete)
                rows.push(tooltipRow("captured", "Spend that day", daily[index - 1] ?? 0));
        }
        else if (actual) {
            rows.push(el("div", { class: "pulse-tooltip-unavailable", text: "Daily captured history unavailable" }));
        }
        if (values.typical !== undefined)
            rows.push(tooltipRow("typical", "Typical pace", values.typical));
        if (values.low !== undefined)
            rows.push(tooltipRow("low", "Low pace", values.low));
        if (values.high !== undefined)
            rows.push(tooltipRow("high", "High pace", values.high));
        if (values.cap !== undefined)
            rows.push(tooltipRow("cap", "Cap pace", values.cap));
        tooltip.replaceChildren(el("p", { class: "pulse-tooltip-date", text: longDate(date) }), el("p", {
    ...
    for (const svg of svgs) {
        ...
        const indexFromPointer = (event) => {
            ...
            if (!(rect.width > 0))
                return selected.get(svg) ?? Math.max(1, elapsed);
            const chartWidth = Number(svg.dataset.chartWidth) || 1;
            const left = Number(svg.dataset.plotLeft) || 0;
            const right = Number(svg.dataset.plotRight) || chartWidth;
        ...
        svg.addEventListener("pointermove", (event) => {
            if (event.pointerType === "touch")
                return;
            show(svg, indexFromPointer(event));
        });
        ...
        svg.addEventListener("pointerleave", (event) => {
            if (event.pointerType !== "touch" && document.activeElement !== svg)
                hide();
        });
...
function forecastDataDisclosure(b) {
    ...
    }, [icon("chevron", { size: 12 }), el("span", { text: "Data & methodology" })]);
...
function forecastChart(b, cov, route) {
...
function spendAnswer(b, cov, route) {
    ...
    if (cap > 0) {
        ...
        answer.appendChild(el("p", { class: `pulse-delta ${tone}` }, [
...
function trustStrip(cov, route) {
    const tone = cov.status === "green" ? "cost-ok" : cov.status === "red" ? "cost-high" : cov.status === "amber" ? "cost-warn" : "muted";
    const channels = [cov.has_proxy ? "proxy" : null, cov.has_otel ? "OTel" : null].filter(Boolean).join(" + ") || "none";
    const label = cov.status === "green"
        ? "Capture healthy"
        : cov.status === "red"
            ? "Capture unavailable"
            : cov.status === "amber"
...
const FILTERABLE_SERIES_DIMENSIONS = new Set([
    "step",
    "component",
    "parent",
    "tool",
...
function anomalySeriesFilter(seriesKey) {
    ...
    if (split === 0 || split === seriesKey.length - 1)
        return null;
    const dimension = seriesKey.slice(0, split);
...
export function attentionSelection(row, scope) {
...
export function driverSelection(match, scope) {
    const selected = match?.cohort_snapshot ?? scope;
    return { ...selected, filters: [...selected.filters] };
}
function selectionLink(selection, fallbackHref, client, context, onSaveError) {
    if (!context)
        return { href: fallbackHref };
    const destination = parseHash(fallbackHref);
    const workspace = destination.name === "optimize" ? "optimize" : "investigate";
    const nextState = { ...context.analysis.get(), workspace, selection };
    const serialized = serializeAnalysisHash(destination.segments, nextState, undefined, destination.query);
    ...
    return {
        ...
        onClick: (event) => {
            const pointer = event;
            if (pointer.button !== 0 || pointer.metaKey || pointer.ctrlKey || pointer.shiftKey || pointer.altKey)
                return;
            event.preventDefault();
            context.analysis.setSelection(selection);
            context.analysis.navigateWorkspace(workspace);
...
function renderAttentionQueue(rows, client, context, onSaveError) {
...
export async function renderPulse(root, client, route, context) {
    let burn;
    let cov;
    let anomalies;
    let budget;
    ...
    try {
        ...
        root.replaceChildren(errorNode("Pulse couldn't load the selected date range.", e, {
            actions: [
    ...
    const reportSaveError = (error, retry) => {
        selectionStatus.replaceChildren(errorNode("Couldn't save this large Selection A, so it was not opened.", error, {
