...
import { renderSvgThemed } from "../svg.js";
...
import { buildRunTimeline, formatTimelineNanos, relatedSteps, } from "./runTimeline.js";
import { aggregatedProfile, applyFrameAction, chronologicalProfile, componentChoices, profileFrames, profileTableRows, sandwichProfile, } from "./runProfileModel.js";
export const RUN_PROFILE_TABS = ["timeline", "profile", "shape", "provenance"];
const TAB_LABEL = {
    timeline: "Timeline",
    profile: "Profile",
    shape: "Shape",
...
function activeTab(route) {
    ...
    return RUN_PROFILE_TABS.includes(view ?? "")
        ? view
        : "profile";
}
function stepEntityId(runId, ordinal) {
    return `step:${encodeURIComponent(runId)}:${ordinal}`;
}
function focusStepForRun(context, runId, steps) {
...
function createStepSelection(context, runId, steps) {
    ...
    return {
        get: () => current,
        select(ordinal) {
            if (!valid.has(ordinal) || current === ordinal)
                return;
            if (context) {
 ...
 */
function runEntityPane(runIds, currentRun, route) {
    const pane = el("aside", {
    ...
    if (currentRun && !ids.includes(currentRun))
        ids.unshift(currentRun);
    ...
    const draw = () => {
        ...
        const start = Math.max(0, Math.min(Math.max(0, filtered.length - windowSize), Math.floor(viewport.scrollTop / RUN_ROW_HEIGHT) - RUN_ROW_OVERSCAN));
        ...
        if (focusedRun)
            queueMicrotask(() => focusLink(focusedRun));
    ...
    viewport.addEventListener("keydown", (event) => {
        const activeId = document.activeElement?.dataset.runId;
...
function paneResizer(pane, controls) {
    return el("div", {
...
function mountRunWorkbench(root, header, entities, canvas, inspector, context, initialPane, selection) {
...
function observedDuration(steps) {
    let start = null;
    let end = null;
    for (const step of steps) {
        if (!step.start_unix_nano || !step.end_unix_nano)
            continue;
        try {
...
function summaryDatum(label, value, detail) {
...
function baselineSummary(state, status, baseline) {
    if (!state?.baseline)
        return { value: "Not selected", detail: "Choose Baseline B in Investigate" };
    if (status.unpriced)
        return { value: "Unavailable", detail: `${state.baseline.label} · run has unpriced usage` };
...
function stickySummary(status, meta, steps, autopsy, quality, baseline) {
...
function whyHeadline(autopsy, status) {
    if (!autopsy) {
        return el("p", {
...
function stepBeamValue(step, key) {
    switch (key) {
...
function runBeam(status, steps, autopsy, selection) {
    const tone = {
        fresh_input: "cat-1",
        cache_read: "cat-2",
        cache_write: "cat-3",
    ...
    const segments = (autopsy?.classes ?? [])
    ...
    for (const segment of segments) {
        ...
        if (candidate)
            candidateByKey.set(segment.key, candidate);
    }
    const beam = tareBeam({
        mode: "run",
        ...
        onSelect: candidateByKey.size > 0
            ? (key) => {
                ...
                if (candidate)
                    selection.select(candidate.ordinal);
            }
            : undefined,
    });
    ...
    for (const button of Array.from(beam.querySelectorAll("button[data-beam-key]"))) {
        const key = button.dataset.beamKey ?? "";
        const candidate = candidateByKey.get(key);
        if (candidate)
            button.title = `Select Step ${candidate.ordinal}, the largest captured ${button.textContent?.split(" · ")[0] ?? key} contributor`;
    }
    selection.subscribe((ordinal) => {
        ...
        const keys = new Set(step ? segments.filter((segment) => stepBeamValue(step, segment.key) > 0).map((segment) => segment.key) : []);
        for (const node of Array.from(beam.querySelectorAll("[data-beam-key]"))) {
...
function whySection(status, steps, autopsy, selection) {
...
function timelineEvidenceCopy(model) {
...
function timelinePanel(model, selection) {
    ...
    const updateSelection = () => {
        for (const row of Array.from(list.querySelectorAll("[data-step-ordinal]"))) {
    ...
    const focusSelected = () => {
        list.querySelector(`[data-step-ordinal="${selected}"] button`)?.focus();
    };
    const draw = () => {
        ...
        list.replaceChildren(...rows);
        paging.replaceChildren(el("button", {
            class: "btn",
            type: "button",
        ...
        }), el("span", { class: "caption sub", text: `Showing ${windowStart + 1}–${windowEnd} of ${ordered.length}` }), el("button", {
            class: "btn",
            type: "button",
...
function fallbackProfileTable(profile) {
...
function profilePanel(flame, profile, selection) {
    ...
    if (!flame) {
        ...
        if (profile?.rows.length)
            panel.appendChild(fallbackProfileTable(profile));
        return panel;
    }
    ...
    let weight = flame.root.micros <= 0 && flame.root.tokens > 0 ? "tokens" : "cost";
    let order = "chronological";
    let tableMode = "cumulative";
    let query = "";
    let componentKey = "";
    let frameTargetKey = "";
    ...
    const weightButtons = ["cost", "tokens"].map((value) => el("button", {
        class: "btn",
        type: "button",
        "data-profile-weight": value,
        text: value === "cost" ? "Cost" : "Tokens",
    }));
    const orderButtons = ["chronological", "aggregated", "sandwich"].map((value) => el("button", {
        class: "btn",
        type: "button",
        "data-profile-order": value,
        text: value[0].toUpperCase() + value.slice(1),
    }));
    const tableButtons = ["flat", "cumulative"].map((value) => el("button", {
        class: "btn",
        type: "button",
    ...
    const actionButtons = ["focus", "ignore", "hide"].map((action) => el("button", {
        class: "btn",
        type: "button",
    ...
    const buildBase = () => {
        if (order === "chronological")
            return chronologicalProfile(flame);
        if (order === "aggregated")
            return aggregatedProfile(flame, weight);
        return componentKey ? sandwichProfile(flame, componentKey, weight) : null;
    };
    const ordinalsFor = (frame) => frame.references
        .filter((ref) => ref.run_id === flame.run_id)
        .map((ref) => ref.step_ordinal)
        .filter((value, index, all) => all.indexOf(value) === index)
        .sort((a, b) => a - b);
    const updateCrossHighlights = () => {
        flameHost.classList.toggle("has-step-selection", selectedOrdinal != null);
        for (const frameNode of Array.from(flameHost.querySelectorAll("[data-step-ordinals]"))) {
        ...
        for (const row of Array.from(tableHost.querySelectorAll("tr[data-step-ordinals]"))) {
    ...
    const chooseFrame = (frame) => {
        ...
        for (const node of Array.from(flameHost.querySelectorAll("[data-frame-key]"))) {
    ...
    const paint = () => {
        ...
        componentSelect.replaceChildren(el("option", { value: "", text: "Select a component…" }), ...choices.map((choice) => el("option", {
        ...
        for (const frame of profileFrames(base).slice(1)) {
            if (!frameOptions.has(frame.frame_key))
                frameOptions.set(frame.frame_key, frame);
        }
        const sortedOptions = [...frameOptions.values()].sort((a, b) => a.node_kind.localeCompare(b.node_kind) || a.display_label.localeCompare(b.display_label));
        frameSelect.replaceChildren(el("option", { value: "", text: "Choose a frame…" }), ...sortedOptions.map((frame) => el("option", {
        ...
        const transformed = frameAction
            ...
            : base;
        const displayed = cullSubPixel(transformed, 1, 960, weight);
        const indexed = profileFrames(displayed);
        flameHost.setAttribute("aria-label", `Interactive ${order} run profile; frame width is ${weight === "cost" ? "estimated cost" : "token"} share`);
        rawSvg(flameHost, renderSvgThemed(displayed, weight));
        const normalizedQuery = query.trim().toLowerCase();
        Array.from(flameHost.querySelectorAll("[data-frame]")).forEach((rect, index) => {
            ...
            if (ordinals.length === 1)
                rect.dataset.stepOrdinal = String(ordinals[0]);
            rect.setAttribute("tabindex", "0");
            rect.setAttribute("role", "button");
            rect.setAttribute("aria-pressed", String(frame.frame_key === selectedFrameKey));
            rect.setAttribute("aria-label", `${frame.display_label}; ${frame.calls} call${frame.calls === 1 ? "" : "s"}; ${fmtUsd(frame.micros)} cumulative; select frame`);
        ...
        tableHost.replaceChildren(el("div", { class: "run-profile-profile-table-head" }, [
...
function transcriptBody(label, text) {
...
function inspectPanel(client, runId, steps) {
    ...
    for (const step of steps) {
        ...
        load.addEventListener("click", () => {
            ...
            void client
                ...
                if (!transcript) {
                    ...
                    result.replaceChildren(emptyState("No bodies captured for this step", "The run used Max inspect, but this capture path or step did not persist a request/response body."));
                    return;
                }
                load.textContent = "Bodies loaded";
                result.replaceChildren(transcript.truncated === true
                    ? el("p", {
                    ...
                    : transcript.truncated === false
                        ...
                        }), transcriptBody("Redacted request", transcript.req), transcriptBody("Redacted response", transcript.resp));
            })
                .catch((error) => {
                load.disabled = false;
        ...
        list.appendChild(el("section", { class: "run-profile-inspect-step" }, [
    ...
    purge.addEventListener("click", () => {
        ...
        void client
            ...
            for (const result of Array.from(list.querySelectorAll("[data-transcript-result]"))) {
                result.replaceChildren(el("p", { class: "caption sub", text: "Purged from the local transcript store." }));
            }
            for (const button of Array.from(list.querySelectorAll("[data-inspect-step]"))) {
...
function shapePanel(client, runId, steps, meta) {
    const panel = el("section", { class: "run-profile-tab-content" }, [
    ...
    for (const step of anatomy) {
        ...
        panel.appendChild(el("section", { class: "run-profile-shape-step" }, [
            el("h3", { text: `Step ${step.ordinal}` }),
            ...
            el("ul", { class: "run-profile-shape-list" }, a.components.map((component) => el("li", {}, [
    ...
    if (meta?.profile === "max_inspect")
        panel.appendChild(inspectPanel(client, runId, steps));
    return panel;
}
...
function tabPanel(tab, client, runId, steps, timeline, flame, profile, meta, selection) {
    const panel = el("div", {
...
function noteSection(client, runId, note) {
    ...
    const starred = el("input", { type: "checkbox" });
...
function exportSection(client, runId) {
    ...
    return el("section", { class: "run-profile-context-section" }, [
        el("h3", { text: "Export" }),
        el("div", { class: "run-profile-context-actions" }, formats.map(([format, extension, label]) => el("button", {
            class: "btn",
            type: "button",
            "data-export": format,
            text: label,
...
function attestationSection(client, runId) {
    const output = el("div", { class: "run-profile-attestation-output", "aria-live": "polite" });
    const maxPrivate = el("input", { type: "checkbox" });
    return el("section", { class: "run-profile-context-section" }, [
...
function selectedStepSection(steps, timeline, selection) {
    ...
    selection.subscribe((ordinal) => {
        ...
        if (!step) {
            section.replaceChildren(el("h3", { id: "run-profile-selected-step-title", text: "Selected step" }), el("p", { class: "caption sub", text: "Select a timeline row, Beam segment, profile frame, or unique table row to inspect one step." }));
            return;
        }
        ...
        if (concurrent.length > 0)
            relationship = `Concurrent sibling span${concurrent.length === 1 ? "" : "s"}: ${stepList(concurrent)}`;
        else if (nested.length > 0)
            relationship = `Nested span overlap: ${stepList(nested)}`;
        else if (overlaps.length > 0)
            relationship = `Timestamp overlap with ${stepList(overlaps)} · no sibling-span relationship`;
        else if (timed)
            relationship = "No timestamp overlap observed";
        const rows = [
        ...
        if (step.trace_id)
            rows.push(["Trace ID", el("code", { text: step.trace_id })]);
        if (step.span_id)
            rows.push(["Span ID", el("code", { text: step.span_id })]);
        if (step.parent_span_id)
            rows.push(["Parent span", el("code", { text: step.parent_span_id })]);
        section.replaceChildren(el("h3", { id: "run-profile-selected-step-title", text: `Selected Step ${step.ordinal}` }), el("dl", { class: "run-profile-selected-step-facts", "aria-live": "polite" }, rows.flatMap(([term, value]) => [el("dt", { text: term }), el("dd", {}, [value])])));
    });
    return section;
}
function contextualInspector(client, runId, note, meta, status, steps, timeline, selection) {
    return el("aside", {
...
export async function renderRunProfile(root, client, route, context) {
    root.querySelector("[data-adaptive-panes]")?.dispatchEvent(new Event("tare:dispose"));
    const runId = route.segments[2];
    if (!runId) {
        root.replaceChildren(errorNode("This Run Profile link is missing a run id.", "Missing run id", {
    ...
    try {
        ...
        root.replaceChildren(errorNode(missing
    ...
    mountRunWorkbench(root, header, runEntityPane(runIds, runId, route), canvas, contextualInspector(client, runId, note, meta, status, steps, timeline, selection), context, "canvas", selection);
}
