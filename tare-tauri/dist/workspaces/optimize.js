...
import { routeHash, routePath, setRouteQuery } from "../ui/store.js";
...
import { renderSavingsVerification, verificationPresentationState, } from "./optimizeVerification.js";
const VIEW_ORDER = [
    "open",
    "applied",
    "verifying",
...
function activeView(raw) {
    if (raw === "observed-reduction" || raw === "observed_reduction")
        return "observed_reduction";
    if (raw === "not-observed" || raw === "not_observed")
        return "not_observed";
    return VIEW_ORDER.includes(raw) ? raw : "open";
}
function actionKey(opportunityKey, hash) {
    return `${opportunityKey}\u001f${hash}`;
}
function derivedState(action, verification) {
    if (!action)
        return "open";
...
function expectedMicros(opportunity, action) {
    return (action?.expected_point_micros ??
        action?.expected_low_micros ??
        opportunity?.recoverable_micros ??
        0);
}
async function loadVerifications(client, actions) {
    const rows = await Promise.all(actions
...
function buildRows(ledger, actions, verifications) {
    const byIdentity = new Map(actions.map((action) => [actionKey(action.opportunity_key, action.cohort_hash), action]));
    ...
    ledger.opportunities.forEach((legacy, index) => {
        if (v2Labels.has(`${legacy.kind}\u001f${legacy.label}`))
            return;
        rows.push({
...
function rowsForView(rows, view) {
    ...
    if (view === "applied")
        return rows.filter((row) => row.action?.status === "applied");
...
function analysisContext(state) {
    const scope = state?.scope;
    const selection = state?.selection;
    const baseline = state?.baseline;
    return el("div", { class: "optimize-analysis-context", role: "group", "aria-label": "Analysis context" }, [
        el("span", {
            class: "caption",
            text: scope
...
function summaryFigure(label, value, detail, className) {
...
function summaries(ledger, rows) {
    ...
    const hasAggregateAssociation = rows.some((row) => row.state === "observed_reduction" &&
    ...
    const node = el("section", { class: "optimize-summary-section", "aria-labelledby": "optimize-summary-h" }, [
        ...
        el("dl", { class: "optimize-summaries" }, [
            summaryFigure("Capped potential", capped, "Estimated current opportunity, bounded by spend; categories may overlap.", "potential"),
            summaryFigure("Applied exposure", applied, "Sum of projected points on applied snapshots; cohorts can overlap and this is not observed.", "applied"),
            summaryFigure(hasAggregateAssociation ? "Observed reduction / association" : "Observed reduction", observed, hasAggregateAssociation
                ? "Sum of positive action-local measurements; aggregate-only rows are associations, and cohorts can overlap."
                : "Sum of positive action-local measurements; overlapping cohorts can overlap here too.", "observed"),
        ]),
        el("p", {
...
function laneValue(rows, predicate) {
    return rows
        .filter(predicate)
        .reduce((sum, row) => sum + Math.max(0, row.expectedMicros), 0);
}
function lifecycleBeam(rows) {
    const open = laneValue(rows, (row) => row.state === "open");
    const verifying = laneValue(rows, (row) => row.state === "verifying");
    const applied = laneValue(rows, (row) => row.action?.status === "applied" && row.state !== "verifying");
    ...
    const beam = tareBeam({
        ...
        onSelect: (key) => setRouteQuery({ view: VIEW_SLUG[key] }),
    });
    beam.appendChild(el("p", {
...
function statePill(state, verification, action) {
    const glyph = {
        open: "○",
        applied: "◆",
        verifying: "◐",
    ...
    const detail = state === "applied" && verification?.failed
...
function actionRequest(opportunity, state) {
    return {
...
function evidenceLink(cohort, label, client, context, status) {
    if (!context)
        return { href: routeHash("investigate") };
    const next = {
...
function copyFix(text, status) {
    ...
    void pending.then(() => showToast("Fix copied", "info"), () => {
        status.textContent = "Couldn't copy the fix. Select the text and copy it manually.";
    });
}
function actionButton(label, action, run, status) {
    return el("button", {
        type: "button",
        class: "btn",
        text: label,
...
function lifecycleRow(row, client, route, context, root, status) {
    ...
    article.appendChild(el("div", { class: "optimize-row-head" }, [
        el("div", {}, [
        ...
        const inspectButton = el("button", {
            ...
            onClick: (event) => {
                ...
                if (loaded)
                    return;
                loaded = true;
                inspector.replaceChildren(el("p", {
                ...
                void client.resolveCohort(exactCohort).then((resolved) => {
                    inspector.replaceChildren(renderOpportunityEvidence({
                        opportunity,
    ...
    if (opportunity && !row.action) {
        actions.append(actionButton("Mark applied", "apply", async () => {
        ...
        }, status), actionButton("Dismiss", "dismiss", async () => {
        ...
        actions.appendChild(actionButton(label, "unaccept", async () => {
    ...
    if (evidenceInspector)
        article.appendChild(evidenceInspector);
    return article;
}
function viewTabs(rows, active, route) {
    const tabs = el("nav", {
...
function queue(rows, view, client, route, context, root, status) {
    ...
    if (visible.length === 0) {
        section.appendChild(emptyState(`No ${VIEW_LABEL[view].toLowerCase()} opportunities`, view === "open"
            ? "Nothing is currently waiting for action in this ledger scope."
            : "No opportunity currently has this lifecycle state.", view === "open"
            ? undefined
...
export async function renderOptimize(root, client, route, context) {
    ...
    try {
        ...
        root.replaceChildren(errorNode("Couldn't load the Optimize lifecycle. Check that capture is running.", error, {
            actions: [
    ...
    root.replaceChildren(el("section", { class: "optimize-workspace", "aria-label": "Optimize" }, [
