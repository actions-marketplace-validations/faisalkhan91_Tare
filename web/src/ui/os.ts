...
export type OsClass = "macos" | "windows" | "linux" | "other";

/// Classify an OS from a userAgent string. Deterministic; case-insensitive.
export function osClass(ua: string): OsClass {
...
export function applyOsClass(
  ...
  const os: OsClass =
    injected === "macos" || injected === "windows" || injected === "linux"
      ? injected
      : osClass(ua);
  root.dataset.os = os;
  if (!root.dataset.material) root.dataset.material = "flat";
  return os;
}
...
export function preferredMaterial(
  os: OsClass,
  isTauri: boolean,
