...
export function skelCardRow(n = 3): HTMLElement {
  return el("div", { class: "cards", "aria-busy": "true" }, Array.from({ length: n }, skelCard));
}

/// `n` line placeholders (a table/list loading state).
export function skelRows(n = 6): HTMLElement {
  return el(
    "div",
    { class: "skel-rows", "aria-busy": "true" },
    Array.from({ length: n }, () => el("div", { class: "skeleton-block skel-row" }))
  );
}

/// A generic screen skeleton: a summary row and a block of result rows.
export function skelScreen(): HTMLElement {
  return el(
    "section",
    {
      class: "section",
