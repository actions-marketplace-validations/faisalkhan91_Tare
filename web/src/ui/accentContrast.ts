...
export function clampAccentL(accent: Oklch, surface: Oklch, floor: number): Oklch | null {
  if (contrastRatio(accent, surface) >= floor) return accent;
  ...
  for (let i = 0; i < 220; i++) {
    ...
    const cand = { ...accent, L };
    if (contrastRatio(cand, surface) >= floor) return cand;
  }
  return null;
}
...
export function pickOnAccent(accent: Oklch): Oklch {
