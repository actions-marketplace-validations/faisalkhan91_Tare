...
/// Minimal shape of the modifier state we read — lets tests pass plain objects, not full events.
export type Chorded = Pick<KeyboardEvent, "key" | "metaKey" | "ctrlKey" | "shiftKey" | "altKey">;
...
export function matchShortcut(e: Chorded, chord: string, os: OsClass = detectOs()): boolean {
  const want = parseChord(chord);
  if (e.key.toLowerCase() !== want.key) return false;
...
export function formatShortcut(chord: string, os: OsClass = detectOs()): string {
  const mac = os === "macos";
  const want = parseChord(chord);
  const mods: string[] = [];
  // Canonical macOS glyph order is ⌃⌥⇧⌘; we only use ⇧/⌥/⌘ here, ⌘ last so "⇧⌘P" reads right.
  if (want.alt) mods.push(mac ? "⌥" : "Alt");
  if (want.shift) mods.push(mac ? "⇧" : "Shift");
  if (want.mod) mods.push(mac ? "⌘" : "Ctrl");
