...
export const GLOSSARY: Record<string, string> = {
  "fresh input":
    "Input tokens sent uncached this turn. The model reads them in full and you pay the full input rate.",
  "cache read":
    "Input tokens served from the provider's prompt cache at a large discount, because you re-sent an identical prefix.",
  "cache write":
    "Input tokens written into the cache this turn. This is a one-time surcharge so later turns can read them cheaply.",
  output: "Tokens the model generated, billed at the higher output rate.",
  reasoning:
    "Hidden 'thinking' tokens some models emit before the answer; billed as output tokens.",
  ttl: "How long a cached prefix lives before it must be re-written. Anthropic offers a 5-minute and a 1-hour tier.",
  "cache erosion":
    "When a changing prefix keeps invalidating the cache, so you pay the cache write surcharge repeatedly instead of cheap reads.",
  prefix:
    "The leading, unchanging part of a prompt (system prompt + tool definitions) that the cache keys on.",
  frontier:
    "The runs where nothing else is both cheaper and higher quality. This is the efficient edge of the cost/quality trade-off.",
  "blended $/1m tokens":
    "Total spend divided by total tokens, scaled to 1M. This is a single efficiency number across a mix of input/output/cache tokens.",
  estimated:
    "Computed from captured token counts and a pricing table. Tare never sees your provider invoice, so every figure is an estimate.",
  flamegraph:
    "A stacked chart where each bar sits inside the bar above it (run → step → prompt component). On screen, width is estimated COST by default — so the widest path is where the money went — with a Tokens mode that weights by token share instead. Exported/report images are always token-weighted.",
};

/// The definition for a term (case-insensitive), or `undefined` if not in the glossary.
export function definitionOf(term: string): string | undefined {
...
export function defineTerm(term: string, label: string = term): HTMLElement {
  const def = definitionOf(term);
  if (!def) return el("span", { text: label });
  uid += 1;
