...
function receiptRow(label: string, value: string, title?: string): HTMLElement {
...
export function receiptStatement(v: ReceiptVerify): HTMLElement {
  return el("dl", { class: "receipt-ledger" }, [
    receiptRow(
      "Recomputed total",
      fmtUsd(v.recomputed_total_micros),
      toDollarString(v.recomputed_total_micros)
    ),
