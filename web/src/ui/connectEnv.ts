...
/// The providers a user can point at Tare. "Point at your provider" should be a 10-second guided
/// choice, not a wall of env vars: the picker shows ONLY the chosen provider's base-URL row(s) plus
/// a one-line "what this does". Presets set base URLs only — never API keys.
...
export interface ProviderPreset {
  ...
  hint: string;
  /// The capture path that fits this provider best ("otel" = the recommended out-of-band path;
  /// "proxy" = the inline base-URL swap).
  capture: "otel" | "proxy";
}
...
export function captureRecommendation(id: string): { mode: "otel" | "proxy"; label: string; why: string } {
...
export function connectEnvFor(id: ProviderId, baseUrl: string): Array<[string, string]> {
...
export function pricingProviderKey(id: ProviderId): string | null {
  switch (id) {
    case "anthropic":
      return "anthropic";
    case "openai":
...
export function pricingFreshnessLine(
  version: string,
  effectiveDate: string,
  id: ProviderId,
  modelsByProvider: Record<string, number> | undefined
): string {
...
export function connectEnvShellFor(id: ProviderId, baseUrl: string): string {
...
export function otelEnv(receiverUrl: string): Array<[string, string]> {
...
export function otelEnvShell(receiverUrl: string): string {
...
export function genericOtelEnvShell(receiverUrl: string): string {
  const u = receiverUrl.replace(/\/+$/, "");
  return [
    ["OTEL_METRICS_EXPORTER", "otlp"],
    ["OTEL_LOGS_EXPORTER", "otlp"],
...
/// A per-agent connect recipe: the "client of choice" onramp beyond Claude Code + Codex.
/// `mode` picks the capture path; `render(otlpUrl, proxyBase)` returns the copy-paste snippet;
/// `note` is the one-line how/where. All additive + reversible — the user pastes env / a settings
/// field, so nothing writes to an agent's config file until per-agent CLI wiring is verified.
export interface AgentRecipe {
  id: string;
  label: string;
  mode: "otel" | "proxy";
  note: string;
  render: (otlpUrl: string, proxyBase: string) => string;
}
...
export function agentRecipe(id: string): AgentRecipe {
