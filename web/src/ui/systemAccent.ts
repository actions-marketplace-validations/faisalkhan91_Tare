...
import { hexToOklch, parseOklch, oklchToCss, clampAccentL, pickOnAccent, type Oklch } from "./accentContrast.js";
...
export function systemAccentEligible(): boolean {
  try {
...
function readSurface(root: HTMLElement): Oklch | null {
  try {
    ...
    const v =
      getComputedStyle(root).getPropertyValue("--surface") || root.style.getPropertyValue("--surface");
...
function clearBridge(root: HTMLElement): void {
...
export function refreshSystemAccent(root: HTMLElement = document.documentElement): void {
...
export function wireSystemAccent(
  ...
  l("system-accent", (e: { payload: unknown }) => {
    const p = e.payload;
    const hex =
      typeof p === "string"
        ? p
        : p && typeof p === "object" && typeof (p as { hex?: unknown }).hex === "string"
          ? (p as { hex: string }).hex
          : null;
