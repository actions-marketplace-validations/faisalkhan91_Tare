...
export interface ParCoordsRow {
  label: string;
  /// One value per axis, in axis order.
  values: number[];
  /// Optional CSS tone class for the polyline stroke.
  tone?: string;
}

export interface ParCoordsOpts {
  width?: number;
  height?: number;
  ariaLabel?: string;
  /// Format an axis's numeric value for its min/max tick. Only the caller knows each
  /// axis's unit ($/tokens/%/ms/…), so it supplies this; return "" to SUPPRESS ticks on an axis where a
  /// numeric min/max is meaningless (a binary or rank axis like cache-control / effort). Without it,
  /// axes render no numeric scale.
  axisFormat?: (axisIndex: number, value: number) => string;
}
...
export function parcoords(axes: string[], rows: ParCoordsRow[], opts: ParCoordsOpts = {}): SVGElement {
  ...
  const axisX = (i: number): number => (n === 1 ? PAD_X : PAD_X + ((W - 2 * PAD_X) * i) / (n - 1));
  const valueY = (i: number, v: number): number => {
