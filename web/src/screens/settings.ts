...
import type { AlertRule, LocalOverlay, TareClient, TareConfigDto } from "../client.js";
...
export async function renderSettings(
  ...
  let cfg: TareConfigDto;
  try {
    cfg = await client.config();
  } catch (e) {
    root.replaceChildren(
      errorNode("Couldn't load settings. Check that tare.toml is readable.", e, {
        actions: [{
          label: "Retry",
          primary: true,
  ...
  let investigations: SavedInvestigationV2[] | null = null;
  try {
  ...
  const origins = await client.configOrigins().catch(() => ({}) as Record<string, string>);
  ...
  const capField = (label: string, control: HTMLElement, path: string): HTMLElement => {
    const origin = origins[path];
    if (origin === "env") {
      (control as HTMLInputElement | HTMLSelectElement).disabled = true;
    }
    const labelText =
      origin === "env"
  ...
  const alertRules: AlertRule[] = (cfg.alert ?? []).map((r) => ({ ...r }));
  const alertList = el("div", { class: "alert-rules" });
  const renderAlertRules = (): void => {
    ...
    alertList.replaceChildren(
      ...alertRules.map((r, i) =>
        el("div", { class: "alert-rule-row" }, [
          el("span", {
            class: "sub",
            text:
              r.metric === "anomaly_kind"
  ...
  renderAlertRules();
  const ruleMetric = el("select", { class: "input", "aria-label": "Alert metric" }, ["today_spend", "period_pct", "run_rate", "anomaly_kind"].map((m) => el("option", { value: m, text: humanizeKey(m) }))) as HTMLSelectElement;
  const ruleValue = el("input", { class: "input", type: "text", placeholder: "Threshold or kind", "aria-label": "Alert threshold or anomaly kind" }) as HTMLInputElement;
  ...
  const overlays: LocalOverlay[] = (cfg.providers.local_overlay ?? []).map((o) => ({ ...o }));
  ...
  renderOverlays();
  const ovBackend = el("select", { class: "input", "aria-label": "Local backend" }, ["ollama", "vllm", "llama.cpp", "tgi", "lmstudio", "localai"].map((b) => el("option", { value: b, text: b }))) as HTMLSelectElement;
  const ovRate = el("input", { class: "input", type: "text", placeholder: "$ / 1M tokens", "aria-label": "USD per 1M tokens" }) as HTMLInputElement;
  ...
  const themeSel = selectEl(
    ["system", "light", "dark"],
    themePreference(),
    (o) => o.charAt(0).toUpperCase() + o.slice(1)
  );
  themeSel.addEventListener("change", () => {
    applyTheme(setThemePreference(themeSel.value as ThemePreference));
  });
  ...
  densitySel.addEventListener("change", () => {
    const d = densitySel.value as Density;
    applyDensity(d);
    setDensity(d);
  });
  // Runs workbench: show/hide the "Change vs baseline" column.
  const baselineDeltaToggle = el("input", { type: "checkbox" }) as HTMLInputElement;
  ...
  const screenSel = el(
    "select",
    { class: "input" },
    SCREENS.map(([v, l]) => el("option", { value: v, text: l }))
  ...
  let bgCloseField: HTMLElement | false = false;
  if (client.canBackgroundOnClose() && currentOs() !== "macos") {
    const bgClose = el("input", { type: "checkbox" }) as HTMLInputElement;
    void client.getBackgroundOnClose().then((on) => {
      bgClose.checked = on;
    });
    bgClose.addEventListener("change", () => void client.setBackgroundOnClose(bgClose.checked));
    bgCloseField = field(
      "Close keeps Tare running in the background",
      bgClose,
      "Desktop close policy · stored on this device"
    );
  }
  const appearanceSection = el(
    "section",
    { class: "section settings-section", "data-settings-section": "appearance" },
    [
      el("h2", {}, ["Appearance"]),
  ...
  const behaviorSection = el(
    "section",
    { class: "section settings-section", "data-settings-section": "behavior" },
    [
      el("h2", {}, ["Behavior"]),
  ...
  const knownProfiles =
    cfg.privacy.profile && !PROFILES.includes(cfg.privacy.profile)
      ? [...PROFILES, cfg.privacy.profile]
      : PROFILES;
  // Humanize the option labels like the sibling selects, keeping the raw enum values.
  const profile = selectEl(["(default)", ...knownProfiles], cfg.privacy.profile ?? "(default)", (o) =>
    o === "(default)" ? "(default)" : humanizeKey(o)
  );
  ...
  const storedMode =
    cfg.capture?.mode && CAPTURE_MODES.includes(cfg.capture.mode) ? cfg.capture.mode : "app_only";
  const captureMode = selectEl(CAPTURE_MODES, storedMode, humanizeKey);
  const CAPTURE_MODE_DESC: Record<string, string> = {
    app_only:
      "Captures only while Tare is open, and catches up everything since you last opened it so nothing is lost.",
    always_on:
      "Installs a background login item so capture keeps running across restarts, even with the window closed.",
    off: "Capture is disabled.",
  };
  const captureModeDesc = el("p", { class: "caption sub" });
  const syncCaptureModeDesc = (): void => {
  ...
  const jsonlEnabled = el("input", { type: "checkbox" }) as HTMLInputElement;
  ...
  if (!canSave) (saveBtn as HTMLButtonElement).disabled = true;
  ...
  const ndToggle = el("input", { type: "checkbox", "aria-label": "Show only non-default config fields" }) as HTMLInputElement;
  ...
  alertSection.append(
    el("h3", { class: "subhead" }, ["Anomaly detection"]),
    el("p", { class: "caption", text: "Defaults for the local spend-anomaly alarm (window 7 days, threshold 50%)." }),
    field("Window (days)", anomalyWindow, "Stored in tare.toml"),
    field("Threshold (%)", anomalyThreshold, "Stored in tare.toml")
  );
  ...
  const shortcutsSection = el(
    "section",
    { class: "section settings-section", "data-settings-section": "shortcuts" },
    [
      el("h2", {}, ["Shortcuts"]),
      ...
      el(
        ...
        shortcutRows.flatMap(([action, shortcut]) => [
          el("dt", { text: action }),
  ...
  const renderInvestigations = (): void => {
    if (investigations === null) {
      investigationList.replaceChildren(
        el("p", { class: "sub", text: "Couldn't read the local investigation database. Retry by reopening Settings." })
      );
      return;
    }
    if (investigations.length === 0) {
      investigationList.replaceChildren(
        el("p", { class: "sub", text: "No saved investigations yet. Save one from Commands while investigating." })
      );
      return;
    }
    investigationList.replaceChildren(
        ...
        const remove = el("button", {
          ...
          onClick: () => {
            ...
            void client
              ...
              .catch(() => {
                (remove as HTMLButtonElement).disabled = false;
  ...
  renderInvestigations();
  const savedInvestigationsSection = el(
    "section",
    { class: "section settings-section", "data-settings-section": "saved-investigations" },
    [
  ...
  if (!canSave) {
    for (const inp of [
      ...
      (inp as HTMLInputElement | HTMLSelectElement | HTMLButtonElement).disabled = true;
    }
  }
  ...
  const settingsNav = el("nav", {
    ...
    const button = el("button", {
      ...
      text: label,
      onClick: () => {
        for (const item of navButtons) item.removeAttribute("aria-current");
  ...
  const earlierSections = el("button", {
    ...
    hidden: "",
  }, [icon("chevron", { size: 14 })]) as HTMLButtonElement;
  ...
  const scrollSections = (direction: -1 | 1): void => {
  ...
  if (typeof requestAnimationFrame === "function") requestAnimationFrame(updateNavOverflow);
  else queueMicrotask(updateNavOverflow);
...
function sectionOrigin(text: string): HTMLElement {
...
function field(
  label: string,
  control: HTMLElement,
  origin = "Stored in tare.toml",
  originClass = "origin-toml"
): HTMLElement {
...
function inputEl(type: string, value: string): HTMLInputElement {
  const i = el("input", { type }) as HTMLInputElement;
  i.value = value;
  return i;
}
function selectEl(
  options: string[],
  value: string,
  labelOf: (o: string) => string = (o) => o
): HTMLSelectElement {
  const s = el(
    "select",
    {},
    options.map((o) => el("option", { value: o, text: labelOf(o) }))
  ) as HTMLSelectElement;
  s.value = value;
  return s;
}
