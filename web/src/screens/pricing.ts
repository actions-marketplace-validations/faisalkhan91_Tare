...
import type { ModelRate, TareClient } from "../client.js";

const rate = (m: number) => el("span", { class: "dollars num", text: fmtUsd(m), title: toDollarString(m) });

export async function renderPricing(root: HTMLElement, client: TareClient): Promise<void> {
  ...
  try {
    ...
    root.appendChild(
      errorNode("Couldn't load pricing. Check that capture is running.", e, {
  ...
  if (models.length === 0) {
    ...
    section.appendChild(
      dataTable<ModelRate>(models, cols, {
  ...
  try {
    ...
    if (report.unpriced && report.unpriced.length > 0) {
      const rows = report.unpriced.map((u) =>
        el("tr", {}, [
