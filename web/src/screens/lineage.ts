...
import type { LineageReport, LineageVersionRow, TareClient } from "../client.js";
...
function lineageSection(rep: LineageReport): HTMLElement {
  ...
  const versionRow = (r: LineageVersionRow): HTMLElement => {
...
export async function renderLineage(root: HTMLElement, client: TareClient): Promise<void> {
  const subtitle = "Cost per prompt/config version: did a rewrite hold quality at lower cost?";
  root.replaceChildren(
    lensSubtitle(subtitle),
    el("section", { class: "section" }, [el("h2", {}, ["Lineage"]), skelRows(4)])
  );

  let reps: LineageReport[];
  try {
  ...
  if (reps.length === 0) {
    root.replaceChildren(
      ...
      el("section", { class: "section" }, [
        el("h2", {}, ["Lineage"]),
        emptyState(
          "No lineages configured",
          "Add [[lineage]] entries to tare.toml. Bind version labels (v1, v2…) to prompt-component fingerprints; then this view compares their cost-per-run.",
          { noAction: true }, // a config task edited in tare.toml — a Connect CTA is a dead end
        ),
      ])
    );
    return;
