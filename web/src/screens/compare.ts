...
import { routePath, currentQuery, setRouteQuery, type Route } from "../ui/store.js";
...
import { flameDiffTree, type FlameDiffOpts } from "../ui/flameDiffView.js";
import type { CauseDelta, FlameDiffModel, ReportDiff, TareClient } from "../client.js";
import { explicitCohortBaseline, pinnedRunBaseline, type UiEntityRef } from "../analysis/state.js";
import type {
  CohortCompareResult,
  CohortFacetResult,
  CohortFilter,
...
export interface CompareRow {
  cause: string;
  /// micro-USD per run, index-aligned with the run id list.
  values: number[];
  total: number;
}

const COMPARE_MODES = ["runs", "cohorts", "versions", "scenarios"] as const;
...
function defaultScope(query: Record<string, string>): CohortSpec {
  return {
...
function replaceIdentityFilter(
  scope: CohortSpec,
  filter: CohortFilter,
  ...
  return {
    ...
    filters: [
      ...scope.filters.filter((existing) => {
        if (existing.op === "run_ids" || existing.op === "step_refs") return false;
        if (
          removeTemplate &&
...
function matchFromState(query: Record<string, string>, stored?: MatchRule): MatchRule {
  if (query.match === "aggregate_only") return { kind: "aggregate_only" };
...
function modeControl(mode: CompareMode): HTMLElement {
  const select = el("select", { "aria-label": "Compare mode" },
    COMPARE_MODES.map((value) => el("option", {
      value,
      selected: value === mode,
...
function runCountLabel(value: number): string {
  return `${value} ${value === 1 ? "run" : "runs"}`;
}

function compareScrollRegion(label: string, table: HTMLElement): HTMLElement {
  return el("div", {
...
function comparisonSummary(
  ...
  results: CohortCompareResult[],
  normalization: CohortSpec["normalization"]
): HTMLElement {
  const baselineResult = results[0]?.baseline;
  const candidateCells = <T>(pick: (result: CohortCompareResult) => T): T[] => results.map(pick);
  const row = (label: string, baselineValue: string, values: string[]) =>
    el("tr", { "data-metric": label.toLowerCase().replace(/ /g, "-") }, [
      el("th", { scope: "row", text: label }),
      el("td", { class: "num", text: baselineValue }),
      ...values.map((value) => el("td", { class: "num", text: value })),
    ]);
  const multipliers = candidateCells((result) =>
    result.baseline.total_micros === 0
      ? "Unavailable"
  ...
  return el("section", {
    ...
    compareScrollRegion("Comparison summary; scroll for candidate columns",
      el("table", { class: "data compare-summary-matrix" }, [
        el("thead", {}, [el("tr", {}, [
          el("th", { text: "Metric" }),
        ...
        el("tbody", {}, [
          row(
            "Sample count",
            baselineResult ? runCountLabel(baselineResult.run_count) : "Unavailable",
            candidateCells((result) => runCountLabel(result.selection.run_count))
          ),
          row(
            "Estimated spend",
            baselineResult ? fmtUsd(baselineResult.total_micros) : "Unavailable",
            candidateCells((result) => fmtUsd(result.selection.total_micros))
          ),
          ...
          row(
            "Compatibility",
            "Reference",
            candidateCells((result) => result.compatibility_warnings.length === 0
              ? "Compatible"
    ...
    ...results.flatMap((result, index) =>
      result.compatibility_warnings.map((warning) => el("p", {
...
function comparedRuns(
  candidates: CompareTarget[],
  results: CohortCompareResult[]
): ComparedRun[] {
  const byId = new Map<string, ComparedRun>();
  const add = (runId: string, role: string): void => {
  ...
  for (const runId of results[0]?.baseline.run_ids ?? []) add(runId, "Baseline B");
  results.forEach((result, index) => {
    for (const runId of result.selection.run_ids) add(runId, `Candidate · ${candidates[index].label}`);
  });
  return [...byId.values()];
}

function qualitySourceLabel(source: string): string {
  if (source === "ci") return "CI";
  if (source === "header") return "capture header";
  if (source === "cli") return "CLI";
  if (source === "ui") return "UI";
  return source;
}

function capturedFrontierSection(
  candidates: CompareTarget[],
  results: CohortCompareResult[],
  ...
  return el("section", { class: "section compare-frontier" }, [
    ...
    })] : [compareScrollRegion("Compared-run quality table",
      el("table", { class: "data compare-frontier-table" }, [
        el("thead", {}, [el("tr", {}, [
...
function missingCount(total: number, missingPct: number): number {
  if (!Number.isFinite(missingPct)) return 0;
  return Math.max(0, Math.min(total, Math.round(total * missingPct / 100)));
}

function trialGroupingSection(
  baseline: CompareTarget,
  candidates: CompareTarget[],
  results: CohortCompareResult[],
  facets: Array<CohortFacetResult | null>
): HTMLElement {
  ...
  return el("section", { class: "section compare-trial-groups" }, [
    ...
    compareScrollRegion("Workload match groups",
      el("table", { class: "data compare-trial-table" }, [
        el("thead", {}, [el("tr", {}, [
...
function representativePairSection(
  candidates: CompareTarget[],
  results: CohortCompareResult[],
  ...
  results.forEach((result, index) => {
    for (const runId of result.selection.run_ids) {
      ...
      if (!roles.includes(candidates[index].label)) roles.push(candidates[index].label);
  ...
  const update = (): void => setRouteQuery({
...
export function compareMatrix(
  ...
  for (const id of runIds) for (const c of Object.keys(perRun[id] ?? {})) causes.add(c);
  const rows: CompareRow[] = [...causes].map((cause) => {
...
export function rowIsFlat(values: number[]): boolean {
...
export function cellFill(value: number, min: number, max: number): string {
  if (max - min <= EPSILON_MICROS) return "transparent";
...
// ---- Cohort volume/size/efficiency decomposition --------------------------------
// The matched comparison API returns three deterministic components:
// volume = entity-count change at baseline size/mix, size = tokens-per-entity change at baseline
// price/mix, efficiency = residual model/cache/unit-price effect. Rust assigns the integer rounding
// remainder to efficiency so the three sum EXACTLY to total_delta_micros — we re-verify that invariant
// client-side so a contract regression is visible rather than silently mis-rendered.

export type DecompositionComponent = "volume" | "size" | "efficiency";
...
export interface DecompositionParts {
  ...
  efficiency: number;
  total: number;
  /// The three components add up to the reported total delta, to the micro-USD.
  sumsExactly: boolean;
  /// Components whose sign OPPOSES the aggregate delta direction — a countervailing move (e.g. the
  /// total rose but efficiency got cheaper). Empty when everything moves with the aggregate.
  reversals: DecompositionComponent[];
}

export function decompositionParts(result: CohortCompareResult): DecompositionParts {
  ...
  const reversals: DecompositionComponent[] = [];
  const check = (name: DecompositionComponent, value: number): void => {
    if (dir !== 0 && value !== 0 && Math.sign(value) !== dir) reversals.push(name);
  };
  check("volume", volume);
  check("size", size);
...
export interface CauseDeltaView {
  /// Causes that got MORE expensive, largest first.
  increases: CauseDelta[];
  /// Causes that got CHEAPER, largest magnitude first.
  decreases: CauseDelta[];
  /// Causes moving OPPOSITE the run-pair's aggregate delta direction.
  reversals: CauseDelta[];
}

export function causeDeltas(diff: ReportDiff, limit = 6): CauseDeltaView {
...
const REFERENCE_MODES = ["baseline", "previous"] as const;
type ReferenceMode = (typeof REFERENCE_MODES)[number];

function referenceControl(mode: ReferenceMode): HTMLElement {
  const select = el("select", { "aria-label": "Decompose relative to" },
    REFERENCE_MODES.map((value) => el("option", {
      value,
      selected: value === mode,
...
function decompositionSection(
  ...
  const blocks = candidates.map((candidate, index) => {
    ...
    const componentRow = (label: string, name: DecompositionComponent, value: number): HTMLElement =>
      el("tr", {
        "data-component": name,
    ...
    return el("div", { class: "compare-decomp-block", "data-candidate": candidate.id }, [
      ...
      compareScrollRegion(`Cost decomposition for ${candidate.label}`,
        el("table", { class: "data compare-decomp-table" }, [
          el("thead", {}, [el("tr", {}, [
...
async function flameDiffSection(
  client: TareClient,
  pairBaseline: string,
  pairCandidate: string,
  opts: FlameDiffOpts
): Promise<HTMLElement | string> {
  if (!pairBaseline || !pairCandidate) return "";
  let model: FlameDiffModel;
  try {
...
async function causeDeltaSection(
  client: TareClient,
  pairBaseline: string,
  pairCandidate: string
): Promise<HTMLElement | string> {
  ...
  const rankList = (title: string, rows: CauseDelta[], kind: string): HTMLElement =>
    el("div", { class: "compare-cause-rank", "data-rank": kind }, [
      el("h3", { text: title }),
      rows.length === 0
...
export async function renderCompare(
  ...
  const requestedMode = query.mode as CompareMode | undefined;
  const mode: CompareMode = requestedMode && COMPARE_MODES.includes(requestedMode)
    ? requestedMode
    : "runs";
  ...
  if (mode === "scenarios") {
    root.replaceChildren(
      modeBar,
      lensSubtitle("Scenarios are evaluated in Optimize, where assumptions and counterfactual provenance stay visible."),
      el("section", { class: "section compare-scenario-handoff" }, [
  ...
  let baseline: CompareTarget | undefined;
  let candidates: CompareTarget[] = [];
  let runIds: string[] = [];
  let orderKey: "runs" | "versions" | undefined;
  ...
  if (mode === "runs") {
    ...
    if (runIds.length === 0 && state?.baseline?.kind === "pinned_run") {
      ...
      if (id) runIds = [id, ...state.comparison.filter((ref) => ref.kind === "run").map((ref) => ref.id)];
    }
    orderKey = "runs";
    if (runIds.length >= 2) {
      baseline = {
    ...
    if (versions.length === 0) {
      const baselineTemplate = state?.baseline?.cohort.filters.find((filter) =>
        filter.op === "eq" && filter.dimension === "template"
      );
    ...
    if (versions.length >= 2) {
      baseline = {
        id: versions[0],
        label: versions[0],
        cohort: replaceIdentityFilter(
          scope,
          { op: "eq", dimension: "template", value: versions[0] },
          true
        ),
      };
  ...
  if (!baseline || candidates.length === 0) {
    ...
    root.replaceChildren(
      modeBar,
      emptyState(title, detail, mode === "runs"
  ...
  const updateMatch = (kind: string, value: string): void => {
    const next: MatchRule = kind === "workload_key"
  ...
  let results: CohortCompareResult[];
  try {
    results = await Promise.all(candidates.map(async (candidate) =>
      (await client.compareCohort({ selection: candidate.cohort, baseline: baseline.cohort, match })).data
    ));
  } catch (error) {
    root.replaceChildren(
      modeBar,
      errorNode("Couldn't build the matched comparison. Adjust the cohort or match rule if it keeps failing.", error, {
        actions: [
  ...
  const reference: ReferenceMode = query.reference === "previous" ? "previous" : "baseline";
  let decompositionResults = results;
  let decompositionError: unknown = null;
  ...
  const frontier = evidence[0]?.status === "fulfilled" ? evidence[0].value as Frontier : null;
  const trialFacets: Array<CohortFacetResult | null> = candidates.map((_, index) => {
    const result = evidence[index + 1];
    return result?.status === "fulfilled"
      ? (result.value as { data: CohortFacetResult }).data
      : null;
  });
  ...
  if (mode === "runs") {
    ...
    context?.analysis.setComparison(candidates.map((candidate): UiEntityRef => ({
  ...
  const setCandidateOrder = (next: CompareTarget[]): void => {
    if (!orderKey) return;
    setRouteQuery({ [orderKey]: [baseline.id, ...next.map((candidate) => candidate.id)].join(",") });
    const kind: UiEntityRef["kind"] = mode === "versions" ? "template" : "run";
  ...
  const decomposition = decompositionSection(
    candidates,
    decompositionResults,
    decompositionError ? "baseline" : reference,
    baseline.label
  );
  ...
  const optimizeQuery: Record<string, string> = { ...query, view: "scenarios" };
  for (const key of [
    "mode", "runs", "versions", "match", "match_value", "pair_baseline", "pair_candidate",
  ]) delete optimizeQuery[key];
  const frontierSection = capturedFrontierSection(
    candidates,
    results,
    frontier,
    routePath(["optimize"], optimizeQuery)
  );
  const comparisonHeader = [
    ...
    flameDiff,
    frontierSection,
  ].filter((node) => node !== null && node !== "") as (HTMLElement | string)[];

  if (mode !== "runs") {
    root.replaceChildren(
      lensSubtitle(`${counted(candidates.length, `${mode === "cohorts" ? "cohort" : "version"} candidate`)} compared against fixed Baseline B.`),
      ...comparisonHeader
    );
    return;
  }
  ...
  let perRun: Record<string, Record<string, number>>;
  try {
    const breakdowns = await Promise.all(
      runIds.map(async (id) => {
        const d = await client.diff(id, id);
        const m: Record<string, number> = {};
        for (const r of d.rows) m[r.cause] = r.micros_before;
    ...
    root.replaceChildren(
      lensSubtitle(`${counted(runIds.length, "run")} compared side by side: where their costs diverge.`),
      ...comparisonHeader,
      errorNode("The matched summary is available, but the legacy cause evidence couldn't load.", e)
    );
    return;
  }
  ...
  const render = (): void => {
    ...
    const head = el("tr", {}, [
      el("th", { text: "Cause" }),
      ...runIds.map((id) =>
        el("th", {}, [el("a", { href: routePath(["investigate", "run", id]), text: id, class: "sub" })])
      ),
    ]);
  ...
  const toggle = el("input", { type: "checkbox", "aria-label": "Differences only" }) as HTMLInputElement;
  ...
  let scatterSection: HTMLElement | string = "";
  try {
  ...
  let parcoordsSection: HTMLElement | string = "";
  try {
    ...
    if (rows.length >= 2) {
      ...
      parcoordsSection = el("section", { class: "section" }, [
        ...
        parcoords(AXES, lines, {
          ...
          axisFormat: (i, v) =>
            i === 0 ? fmtTokens(v) : i === 1 || i === 2 ? `${Math.round(v)}%` : fmtUsd(v),
        }),
