...
import type { RunStep } from "../client.js";
...
export interface RunTimelineModel {
  mode: "step_order" | "timeline";
  orderedSteps: RunStep[];
  timedSteps: TimedRunStep[];
  timedByOrdinal: Map<number, TimedRunStep>;
  timedCount: number;
  totalCount: number;
  origin: bigint | null;
  end: bigint | null;
...
function decimalNanos(value: string | undefined): bigint | null {
...
function overlaps(a: TimedRunStep, b: TimedRunStep): boolean {
...
function countOverlaps(sorted: TimedRunStep[]): number {
  ...
  for (let index = 0; index < spans.length; index++) {
    ...
    while (low < high) {
      ...
      if (spans[middle].start < end) low = middle + 1;
      else high = middle;
    }
...
function sameTrace(a: RunStep, b: RunStep): boolean {
  return Boolean(a.trace_id && b.trace_id && a.trace_id === b.trace_id);
}

function isNested(a: RunStep, b: RunStep): boolean {
  if (!sameTrace(a, b) || !a.span_id || !b.span_id) return false;
  return a.parent_span_id === b.span_id || b.parent_span_id === a.span_id;
}

function isSiblingConcurrency(a: RunStep, b: RunStep): boolean {
...
export function buildRunTimeline(steps: RunStep[]): RunTimelineModel {
  const captured = [...steps].sort((a, b) => a.ordinal - b.ordinal);
  const parsed: Array<{ step: RunStep; start: bigint; end: bigint }> = [];
  ...
  const timed = parsed
    .map<TimedRunStep>(({ step, start, end: stepEnd }) => ({
      step,
      start,
      end: stepEnd,
      offset: start - origin,
  ...
  const siblingGroups = new Map<string, Map<string, TimedRunStep>>();
  for (const row of timed) {
    const step = row.step;
    if (!step.trace_id || !step.parent_span_id || !step.span_id) continue;
    const key = `${step.trace_id}\u0000${step.parent_span_id}`;
    const group = siblingGroups.get(key) ?? new Map<string, TimedRunStep>();
    if (!group.has(step.span_id)) group.set(step.span_id, row);
    siblingGroups.set(key, group);
  }
  let concurrentCount = 0;
  for (const group of siblingGroups.values()) {
    concurrentCount += countOverlaps([...group.values()].sort((a, b) =>
      a.start < b.start ? -1 : a.start > b.start ? 1 : a.step.ordinal - b.step.ordinal
    ));
  }

  // Direct parent-child nesting is linear once trace/span identity is indexed.
  const byTraceSpan = new Map<string, TimedRunStep>();
  for (const row of timed) {
  ...
  for (const row of timed) {
    const step = row.step;
    if (!step.trace_id || !step.parent_span_id) continue;
    const parent = byTraceSpan.get(`${step.trace_id}\u0000${step.parent_span_id}`);
    if (parent && overlaps(row, parent)) nestedCount++;
  }
...
export function formatTimelineNanos(nanos: bigint): string {
  if (nanos <= 0n) return "0 ms";
  if (nanos < 1_000_000n) return "<1 ms";
  const micros = nanos / 1_000n;
  if (micros < 1_000_000n) {
    const wholeMs = micros / 1_000n;
    const tenth = (micros % 1_000n) / 100n;
    return tenth > 0n ? `${wholeMs}.${tenth} ms` : `${wholeMs} ms`;
  }
  const tenths = nanos / 100_000_000n;
  if (tenths < 600n) return `${tenths / 10n}.${tenths % 10n} s`;
  const wholeSeconds = nanos / 1_000_000_000n;
...
export function relatedSteps(
  ...
  ordinal: number,
  relation: "overlap" | "nested" | "concurrent"
): number[] {
  const selected = model.timedByOrdinal.get(ordinal);
  if (!selected) return [];
  return model.timedSteps
    .filter((candidate) => {
      if (candidate.step.ordinal === ordinal || !overlaps(selected, candidate)) return false;
      if (relation === "nested") return isNested(selected.step, candidate.step);
      if (relation === "concurrent") return isSiblingConcurrency(selected.step, candidate.step);
      return true;
    })
