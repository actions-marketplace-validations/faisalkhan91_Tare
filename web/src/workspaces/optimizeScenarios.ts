...
function canonicalValues(raw: string): string[] {
  return [...new Set(raw.split(/[\n,]/).map((value) => value.trim()).filter(Boolean))]
    .filter((value) => value !== AS_CAPTURED)
    .sort((a, b) => a.localeCompare(b));
}

function addCandidate(input: HTMLInputElement, value: string): void {
  const values = canonicalValues(`${input.value},${value}`);
  input.value = values.join(", ");
  input.focus();
}

function lifecycleHref(route: Route): string {
...
function scenarioContext(
  cohort: CohortSpec,
  source: "Selection A" | "Scope",
  resolved: AnalysisResponse<CohortResolveResult> | null
): HTMLElement {
  ...
  section.append(
    el("dl", { class: "scenario-provenance-grid" }, [
  ...
  if (provenance.assumptions.length > 0) {
    section.appendChild(
      el("ul", { class: "scenario-assumptions", "aria-label": "Scenario input assumptions" },
        provenance.assumptions.map((assumption) => el("li", { class: "caption", text: assumption })))
    );
  }
  return section;
}

function renderCacheAdvice(
  host: HTMLElement,
  advice: CacheAdvice[] | null,
  decache: HTMLInputElement
): void {
  ...
  if (advice === null) {
    ...
    section.appendChild(
      el("p", { class: "caption", text: "No stable repeated prefix currently meets the advice detector's threshold." })
    );
  } else {
    const rows = [...advice]
...
function renderWhatIfBody(
  host: HTMLElement,
  result: WhatIfRecommendations | null,
  ...
  if (result.recommendations.length === 0) {
    host.replaceChildren(
      el("p", { class: "caption", text: "No priced alternative model is available for this compatibility view." })
    );
    return;
  }
  const rows = [...result.recommendations]
    ...
    .slice(0, 6);
  host.replaceChildren(
    el("p", { class: "caption", text: `Whole-store baseline ${fmtUsd(result.baseline_micros)} · every model swap is an approximate tokenizer-preserving reprice.` }),
    el("table", { class: "data scenario-compact-table" }, [
      el("thead", {}, [el("tr", {}, [
...
function modelWhatIfPanel(
  client: TareClient,
  initial: WhatIfRecommendations | null,
  modelInput: HTMLInputElement
): HTMLElement {
  ...
  cross.addEventListener("change", () => {
    ...
    void client.whatif(cross.checked).then(
      (result) => renderWhatIfBody(body, result, modelInput),
      () => renderWhatIfBody(body, null, modelInput)
    );
  });
...
function frontierPanel(
  ...
  const updateCompare = (): void => {
    ...
    if (ids.length >= 2) compare.removeAttribute("aria-disabled");
    else compare.setAttribute("aria-disabled", "true");
  };
  ...
  const points = [...frontier.points].sort(
    (a, b) => a.cost_micros - b.cost_micros || a.run_id.localeCompare(b.run_id)
  );
  const body = el("tbody");
  for (const point of points) {
    ...
    checkbox.addEventListener("change", () => {
      if (checkbox.checked) selected.add(point.run_id);
      else selected.delete(point.run_id);
      updateCompare();
    });
...
function optionalInteger(input: HTMLInputElement, label: string): { value?: number; error?: string } {
  if (input.value.trim() === "") return {};
  const value = Number(input.value);
  if (!Number.isSafeInteger(value)) return { error: `${label} must be a whole-number quality score.` };
  return { value };
}

function isBaselineCell(result: ExperimentResult, index: number): boolean {
  return result.cells[index]?.coords.every((coord) => {
    if (coord.axis === "cache_strategy") return coord.value === false;
    return coord.value === null;
  }) ?? false;
}

function renderExperimentResult(host: HTMLElement, result: ExperimentResult): void {
  if (result.cells.length === 0) {
    host.replaceChildren(
      emptyState(
        "No priced experiment cells",
        "The quality gate may exclude every captured run, or every requested target may be unpriced. Tare does not fabricate a $0 result."
      )
    );
    return;
  }
  ...
  host.replaceChildren(
    el("section", { class: "scenario-results", "aria-labelledby": "scenario-results-h" }, [
      el("div", { class: "scenario-section-head" }, [
        el("div", {}, [
...
function experimentBuilder(
  ...
  client: TareClient,
  modelInput: HTMLInputElement,
  snapshotsInput: HTMLInputElement,
  decache: HTMLInputElement
): HTMLElement {
  ...
  run.addEventListener("click", () => {
    ...
    const axes: Axis[] = [];
    if (models.length > 0) axes.push({ kind: "model", values: [AS_CAPTURED, ...models] });
    if (snapshots.length > 0) {
      axes.push({ kind: "pricing_snapshot", values: [AS_CAPTURED, ...snapshots] });
    }
    if (decache.checked) axes.push({ kind: "cache_strategy", values: [AS_CAPTURED, "decache"] });
    if (axes.length === 0) {
    ...
    void client.runExperiment(request).then(
      (result) => {
      ...
      (error) => {
        ...
        resultHost.replaceChildren(
          errorNode("Couldn't run the offline experiment.", error, {
...
export async function renderOptimizeScenarios(
  root: HTMLElement,
  client: TareClient,
  route: Route,
  ...
  root.replaceChildren(
    el("section", { class: "optimize-workspace optimize-scenarios", "aria-label": "Optimize scenarios" }, [
