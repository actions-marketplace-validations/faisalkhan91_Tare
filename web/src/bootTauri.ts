...
export function wireNativeNav(
  listen?: (event: string, cb: (e: { payload: unknown }) => void) => void
): void {
  // eslint-disable-next-line @typescript-eslint/no-explicit-any
  const l = listen ?? (globalThis as any).__TAURI__?.event?.listen;
  if (typeof l !== "function") return;
  const navigateRoute = (route: string, param?: string): void => {
  ...
  const handle = (e: { payload: unknown }): void => {
    const p = e.payload;
    if (typeof p === "string" && p) {
      navigateRoute(p);
    } else if (p && typeof p === "object") {
      const o = p as { route?: unknown; param?: unknown; hash?: unknown };
...
export function wireFullscreen(getWin?: () => { isFullscreen?: () => Promise<boolean>; onResized?: (cb: () => void) => void } | undefined): void {
  // eslint-disable-next-line @typescript-eslint/no-explicit-any
  const w = (globalThis as any).__TAURI__?.window;
  const cur = getWin?.() ?? w?.getCurrentWindow?.() ?? w?.getCurrent?.();
  if (!cur?.isFullscreen) return;
  const sync = (): void => {
    cur
      .isFullscreen!()
      .then((fs: boolean) => {
        if (fs) document.documentElement.dataset.fullscreen = "true";
...
export function applyA11yTraits(root: HTMLElement = document.documentElement): {
...
export function wireWindowActive(
  getWin?: () => { onFocusChanged?: (cb: (e: { payload: boolean }) => void) => void } | undefined,
  ...
  const set = (focused: boolean): void => {
    if (focused) delete root.dataset.inactive;
    else root.dataset.inactive = "true";
  };
  set(true); // assume active on load
  cur?.onFocusChanged?.((e: { payload: boolean }) => set(!!e.payload));
}
...
export function consumeShellInit(root: HTMLElement = document.documentElement): void {
  const init = (globalThis as unknown as { __TARE_SHELL_INIT__?: { traffic_light_reserve_px?: number } })
    .__TARE_SHELL_INIT__;
